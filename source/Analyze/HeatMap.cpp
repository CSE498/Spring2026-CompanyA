...
cse498::HeatMap::HeatMap(ActionLog log, std::pair<int, int> ideal_heatmap_size, std::pair<double, double> map_size) :
    m_locations(ParseActionLog(log)), m_ideal_heatmap_size(ideal_heatmap_size), m_map_size(map_size) {
    ...
    if (invalid_pair(heatmap_rows, heatmap_cols)) {
        throw std::invalid_argument("HeatMap: ideal_heatmap_size must have positive dimensions");
    }

    if (invalid_pair(map_width, map_height)) {
        throw std::invalid_argument("HeatMap: total_gridsize must have positive dimensions");
    }
...
std::vector<std::vector<int>> cse498::HeatMap::OutPutHeatMap(std::ostream& output) {
    const auto [heatmap_rows, heatmap_cols] = m_ideal_heatmap_size;
...
std::vector<std::vector<int>> cse498::HeatMap::FillGrid() {
    ...
    auto in_bounds = [map_width, map_height](double x, double y, double grid_x, double grid_y) {
        return x < map_width && y < map_height && grid_x >= 0 && grid_y >= 0;
    };
...
std::vector<std::pair<double, double>> cse498::HeatMap::GetInvalidInputs() { return m_invalid_locations; }

std::vector<std::pair<double, double>> cse498::HeatMap::ParseActionLog(ActionLog log) {
    std::vector<std::pair<double, double>> locations;
    for (auto& action: log.GetActions()) {
        // Only movement actions contribute coordinates to the heatmap.
        if (action.ActionType == "move") {
            locations.push_back(std::pair<double, double>{action.Position.X(), action.Position.Y()});
        }
    }
    return locations;
}
