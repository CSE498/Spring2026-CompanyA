...
struct ActionSummary 
{
    std::string key; // Stable identifier for the action summary, e.g., "most_active_entity"
...
public:
     ...
     * @return Combined snapshot of numeric series summaries and action summaries.
     */
    DashboardSnapshot BuildSnapshot(const AnalyticsManager& analytics, const ActionLog& actions) const;
     ...
     */
    std::optional<StatSummary> BuildSeriesSummary(const std::string& key, const std::string& label, const DataLog& log) const;
     ...
     */
    std::optional<ActionSummary> BuildActionSummary(const std::string& key, const std::string& label, const ActionLog& log) const;
};
