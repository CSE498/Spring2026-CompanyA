 ...
 */
std::optional<StatSummary>
StatsTracker::BuildSeriesSummary(const std::string &key,
...
StatsTracker::BuildSnapshot(const AnalyticsManager &analytics) const {
  ...
  if (auto summary = BuildSeriesSummary("enemies_tracked", "Enemies Tracked",
                                        analytics.GetEnemiesTrackedLog());
      summary.has_value()) {
    snapshot.numericStats.push_back(*summary);
  }

  if (auto summary = BuildSeriesSummary("damage_dealt", "Damage Dealt",
 ...
 */
std::optional<ActionSummary>
StatsTracker::BuildActionSummary(const std::string &key,
                                 const std::string &label,
