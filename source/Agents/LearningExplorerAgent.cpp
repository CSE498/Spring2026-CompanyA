...
namespace cse498
{
     ...
     */
    LearningExplorerAgent::LearningExplorerAgent(size_t id, const std::string &name, const WorldBase &world) : AgentBase(id, name, world) {}

    /**
     * Initializes the agent by verifying required movement actions exist.
     *
     * @return True if all required actions ("up", "down", "left", "right") are available.
     */
     ...
     *
     * Increments visit count for the current position and tracks initial turn state.
     *
     * @param grid The world grid.
     */
    ...
    WorldPosition LearningExplorerAgent::BFSNextStep(const WorldGrid &grid) const
    {
        ...
        while (!frontier.empty())
        {
            ...
            if (cur_idx != start_idx)
            {
                ...
                if (it == visit_count_by_cell.end() && agent_cells.find(cur_idx) == agent_cells.end())
                {
     ...
     */
    double LearningExplorerAgent::ScoreAction(const WorldGrid &grid, size_t action_id, const WorldPosition &bfs_target) const
    {
        const WorldPosition current_pos = GetLocation().AsWorldPosition();
    ...
    size_t LearningExplorerAgent::SelectAction(const WorldGrid &grid)
    {
        ...
        const std::vector<size_t> candidate_actions = {
            GetActionID("up"), GetActionID("down"), GetActionID("left"), GetActionID("right")
        };
     ...
     */
    int LearningExplorerAgent::GetVisitedCellCount(const WorldGrid &grid, WorldPosition pos) const { return GetVisitCount(grid, pos); }
} // namespace cse498
