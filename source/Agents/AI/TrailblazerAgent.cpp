...
std::optional<size_t> TrailblazerAgent::NextMoveToward(const WorldGrid& grid, WorldPosition target,
    ...
    const auto is_walkable_for_path = [&](WorldPosition pos) {
        if (!IsWalkable(grid, pos))
            return false;
        if (!allow_enemy_target && ai_world->IsEnemyAtPosition(pos))
            return false;
        return true;
    };
    ...
    while (!frontier.empty()) {
        ...
        for (const auto& neighbor: neighbors) {
            if (visited.contains(neighbor))
                continue;
            if (!is_walkable_for_path(neighbor))
                continue;
    ...
    if (!parent.contains(step) && !(step == target))
        return std::nullopt;
...
std::optional<size_t> TrailblazerAgent::ExploreMove(const WorldGrid& grid) const {
    ...
    for (const auto& [pos, action]: options) {
        if (!IsWalkable(grid, pos))
            continue;
        if (ai_world && ai_world->IsEnemyAtPosition(pos))
            continue;
...
std::optional<size_t> TrailblazerAgent::ChaseEnemyMove(const WorldGrid& grid, WorldPosition enemy_pos) const {
    ...
    while (!frontier.empty()) {
        ...
        const int d = static_cast<int>(std::abs(static_cast<int>(pos.CellX()) - static_cast<int>(enemy_pos.CellX())) +
        ...
        for (const auto& n: neighbors) {
            ...
            if (!IsWalkable(grid, n))
                continue;
            if (ai_world->IsEnemyAtPosition(n))
                continue;
            visited.insert(n);
            parent[n] = pos;
            frontier.push(n);
    ...
    if (!best_reachable.has_value() || *best_reachable == start)
        return std::nullopt;
...
std::vector<TrailblazerAgent::PlanAction> TrailblazerAgent::BuildPlan(const WorldGrid& grid) const {
    ...
    for (const auto& item: items) {
        if (item.mKind == LootItem::ItemKind::Heal)
            heal_visible = true;
        if (item.mKind == LootItem::ItemKind::Weapon)
            weapon_visible = true;
        if (item.mPosition == current && item.mKind == LootItem::ItemKind::Heal)
            at_heal_item = true;
        if (item.mPosition == current && item.mKind == LootItem::ItemKind::Weapon)
            at_weapon_item = true;
    }
    ...
    uint32_t start = 0;
    if (my_state.mHP <= kLowHpThreshold)
        start |= LOW_HP;
    if (my_state.mHealCharges > 0)
        start |= HAS_HEAL;
    if (my_state.mAttack > 2)
        start |= HAS_WEAPON;
    if (at_heal_item)
        start |= AT_HEAL_ITEM;
    if (at_weapon_item)
        start |= AT_WEAPON_ITEM;
    if (AttackActionForAdjacentEnemy().has_value())
        start |= ENEMY_ADJ;
    if (enemy_alive)
        start |= ENEMY_ALIVE;
    if (heal_visible)
    ...
    while (!frontier.empty()) {
        ...
        if (MeetsGoal(node.state, goal_mask, goal_value))
            return node.plan;
        if (node.plan.size() >= 5)
            continue;

        for (const auto& action: actions) {
            if ((node.state & action.required_mask) != action.required_value)
                continue;
            ...
            if (best_cost.contains(next_state) && best_cost[next_state] <= next_cost)
                continue;
