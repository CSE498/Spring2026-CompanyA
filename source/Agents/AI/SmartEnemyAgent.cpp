...
/**
 * @brief Computes the shortest path distance to a target using breadth-first search (BFS).
 * Helper function
 *
 * @param grid The world grid used for pathfinding and validity checks.
 * @param target The destination position to compute distance to.
 * @return The number of steps in the shortest path as a std::optional<size_t>,
 * or std::nullopt if no path exists or the agent has no valid position.
 */
std::optional<size_t> SmartEnemyAgent::BfsDistanceTo(const WorldGrid& grid, WorldPosition target) const {
    if (!GetLocation().IsPosition())
        return std::nullopt;
...
std::optional<size_t> SmartEnemyAgent::ShortestAttackLaneMove(const WorldGrid& grid, WorldPosition target) const {
    ...
    for (const auto& tile: attack_tiles) {
        ...
        if (ai_world && ai_world->IsAgentAtPosition(tile, GetID()) && tile != GetLocation().AsWorldPosition()) {
            continue;
        }

        const auto dist = BfsDistanceTo(grid, tile);
        if (!dist.has_value())
            continue;
...
size_t SmartEnemyAgent::SelectAction(const WorldGrid& grid) {
    ...
    if (const auto tracked = world.GetTrackedPlayerPosition(); tracked.has_value()) {
        const int dist =
                static_cast<int>(std::abs(static_cast<long>(current.CellX()) - static_cast<long>(tracked->CellX())) +
