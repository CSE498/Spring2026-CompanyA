...
namespace cse498 {
MerchantAgent::MerchantAgent(std::size_t id, const std::string& name, WorldBase& world) : AgentBase(id, name, world) {
    // Merchant starts empty by default and owning world decides what merchant sells and how
    // much gold it starts with
}

std::size_t MerchantAgent::SelectAction([[maybe_unused]] const WorldGrid& grid) { return 0; }

bool MerchantAgent::IsAvailableForTrade() const { return mAvailableForTrade; }

void MerchantAgent::SetAvailableForTrade(bool available) { mAvailableForTrade = available; }

const std::vector<TradeOffer>& MerchantAgent::GetOffers() const { return mOffers; }

const TradeOffer* MerchantAgent::FindOffer(const std::string& itemName) const {
    // Search the offer list by item name
    const auto it =
            std::ranges::find_if(mOffers, [&itemName](const TradeOffer& offer) { return offer.mItemName == itemName; });

    return (it == mOffers.end()) ? nullptr : &(*it);
}

void MerchantAgent::AddInitialOffer(const TradeOffer& offer) { AddOffer(offer); }

void MerchantAgent::ClearInitialOffers() { ClearOffers(); }
...
TradeResult MerchantAgent::BuyFromMerchant(PlayerAgent& player, const std::string& itemName, std::size_t quantity) {
    ...
    if (!mAvailableForTrade) {
        return {TradeStatus::MerchantClosed, GetTradeClosedMessage(), itemName, quantity, 0};
    }

    // Find if the offer is in the merchant's offer list
    TradeOffer* offer = FindOfferMutable(itemName);
    if (offer == nullptr) {
        return {TradeStatus::UnknownItem, "I do not sell that item.", itemName, quantity, 0};
    }

    // delegate transaction rules to TradeSystem
    return TradeSystem::BuyItem(GetWorld(), player, *this, *offer, quantity);
}
...
TradeResult MerchantAgent::SellToMerchant(PlayerAgent& player, const std::string& itemName, std::size_t quantity) {
    ...
    if (quantity == 0) {
        return {TradeStatus::InvalidQuantity, "Quantity must be at least 1.", itemName, 0, 0};
    }
    ...
    if (item == nullptr) {
        return {TradeStatus::PlayerOutOfStock, "You do not have that item.", itemName, quantity, 0};
    }
    ...
    return TradeSystem::SellItem(GetWorld(), player, *this, offer, quantity);
}
...
TradeOffer& MerchantAgent::EnsureLimitedOffer(const std::string& itemName, std::size_t buyPrice, std::size_t sellPrice,
                                              int itemValue) {
    ...
    mOffers.push_back(TradeOffer{.mItemName = itemName,
                                 .mBuyPrice = buyPrice,
...
void MerchantAgent::ClearOffers() { mOffers.clear(); }

void MerchantAgent::AddOffer(const TradeOffer& offer) { mOffers.push_back(offer); }
} // namespace cse498
