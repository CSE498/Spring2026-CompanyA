...
public:
     ...
     */
    static TradeResult BuyItem(const WorldBase& world, PlayerAgent& player, MerchantAgent& merchant, TradeOffer& offer,
                               std::size_t quantity);
     ...
     */
    static TradeResult SellItem(const WorldBase& world, PlayerAgent& player, MerchantAgent& merchant, TradeOffer& offer,
                                std::size_t quantity);
    ...
    static TradeResult BuyItem(PlayerAgent& player, MerchantAgent& merchant, TradeOffer& offer, std::size_t quantity,
                               Args&&... args) {
        ...
        if (!offer.Consume(quantity)) {
            ...
            return {TradeStatus::MerchantOutOfStock, "Merchant stock changed. Try again.", offer.mItemName, quantity,
                    0};
        }
        ...
        if (playerInventory.AddItem<ITEM_T>(quantity, std::forward<Args>(args)...) != 0) {
            ...
            return {TradeStatus::InventoryFull, "Your inventory is full.", offer.mItemName, quantity, 0};
        }

        merchant.AddGold(totalCost);

        return {TradeStatus::Success,
                "Bought " + std::to_string(quantity) + " " + offer.mItemName + " for " + std::to_string(totalCost) +
                        " gold.",
                offer.mItemName, quantity, totalCost};
    }
    ...
    template<typename ITEM_T, typename... Args>
        requires std::derived_from<ITEM_T, Item>
    static TradeResult SellItem(PlayerAgent& player, MerchantAgent& merchant, TradeOffer& offer, std::size_t quantity,
                                Args&&... args) {
        Inventory& playerInventory = player.GetInventory();
        ...
        if (playerInventory.GetTotal(offer.mItemName) < quantity) {
            return {TradeStatus::PlayerOutOfStock, "You do not have enough to sell.", offer.mItemName, quantity, 0};
        }

        const std::size_t payout = offer.mSellPrice * quantity;
        if (merchant.GetGold() < payout) {
            return {TradeStatus::MerchantCannotAfford, "Merchant does not have enough gold.", offer.mItemName, quantity,
                    payout};
        }
        ...
        if (playerInventory.RemoveItem(offer.mItemName, quantity, true) != 0) {
            return {TradeStatus::PlayerOutOfStock, "Could not remove items from your inventory.", offer.mItemName,
                    quantity, 0};
        }
        ...
        if (!merchant.SpendGold(payout)) {
            ...
            return {TradeStatus::MerchantCannotAfford, "Merchant gold changed. Try again.", offer.mItemName, quantity,
                    0};
        }
        ...
        return {TradeStatus::Success,
                "Sold " + std::to_string(quantity) + " " + offer.mItemName + " for " + std::to_string(payout) +
                        " gold.",
                offer.mItemName, quantity, payout};
    }
};
} // namespace cse498
