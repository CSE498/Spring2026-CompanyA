...
bool FarmingAgent::IsAtPosition(const WorldPosition& position) const { return GetPosition() == position; }
...
std::optional<WorldPosition> FarmingAgent::FindAdjacentWorkTile(const WorldGrid& grid) const {
    ...
    const std::array<WorldPosition, 4> candidates = {buildingPosition.Up(), buildingPosition.Down(),
                                                     buildingPosition.Left(), buildingPosition.Right()};
...
std::size_t FarmingAgent::ChooseNextPathAction(const WorldPath& path) const {
    ...
    if (nextPosition == currentPosition.Up())
        return GetActionID("up");
    if (nextPosition == currentPosition.Down())
        return GetActionID("down");
    if (nextPosition == currentPosition.Left())
        return GetActionID("left");
    if (nextPosition == currentPosition.Right())
