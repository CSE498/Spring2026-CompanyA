...
namespace {
    ...
    bool EnsureElementAvailable(const val& element, const std::string& id, const char* action) {
        if (!element.isNull() && !element.isUndefined()) {
            return true;
        }

        const std::string resolved_id = id.empty() ? "<uninitialized>" : id;
        cse498::GetConsole().call<void>("warn", std::string("WebImage '") + resolved_id + "' skipped " + action +
                                                        " because its DOM element is unavailable.");
        return false;
    }
...
namespace cse498 {
    ...
    void WebImage::UnregisterImage(int id) { sImageRegistry.erase(id); }

    void WebImage::UpdateRegistryEntry(int id, WebImage* img) { sImageRegistry[id] = img; }
    ...
    val WebImage::GetDocument() { return val::global("document"); }

    /// Converts an integer to a CSS pixel string, e.g. 42 -> "42px".
    std::string WebImage::ToPx(int value) { return std::to_string(value) + "px"; }
    ...
    WebImage::WebImage(const std::string& src, const std::string& alt_text) : mSrc(src), mAltText(alt_text) {
    ...
    WebImage::~WebImage() { CleanupElement(); }

    /// Move-constructs a WebImage, transferring DOM ownership from other.
    WebImage::WebImage(WebImage&& other) noexcept :
        mSrc(std::move(other.mSrc)), mAltText(std::move(other.mAltText)), mWidth(other.mWidth), mHeight(other.mHeight),
        mOpacity(other.mOpacity), mIsVisible(other.mIsVisible), mIsLoaded(other.mIsLoaded), mHasError(other.mHasError),
        mErrorMode(other.mErrorMode), mPlaceholderColor(std::move(other.mPlaceholderColor)),
        mOnLoadCallback(std::move(other.mOnLoadCallback)), mOnErrorCallback(std::move(other.mOnErrorCallback)),
    ...
    std::string WebImage::GetSource() const { return mSrc; }
    ...
    std::string WebImage::GetAltText() const { return mAltText; }
    ...
    int WebImage::GetWidth() const { return mWidth; }

    /// Returns the current display height.
    int WebImage::GetHeight() const { return mHeight; }
    ...
    double WebImage::GetOpacity() const { return mOpacity; }
    ...
    bool WebImage::IsVisible() const { return mIsVisible; }

    // ----- Loading State & Error Handling -----

    /// Manually sets the loaded flag (used for tracking async load state).
    void WebImage::MarkLoaded(bool loaded) { mIsLoaded = loaded; }

    /// Returns true if the image has finished loading.
    bool WebImage::IsLoaded() const { return mIsLoaded; }

    /// Returns true if the image source failed to load.
    bool WebImage::HasError() const { return mHasError; }

    /// Registers a callback invoked when the image loads successfully.
    void WebImage::SetOnLoadCallback(std::function<void()> callback) { mOnLoadCallback = std::move(callback); }

    /// Registers a callback invoked when the image fails to load.
    void WebImage::SetOnErrorCallback(std::function<void()> callback) { mOnErrorCallback = std::move(callback); }

    /// Configures the error-handling strategy (BlankRect or NoOp).
    void WebImage::SetErrorMode(ImageErrorMode mode) { mErrorMode = mode; }

    /// Sets the CSS color used for the BlankRect placeholder.
    void WebImage::SetPlaceholderColor(const std::string& css_color) { mPlaceholderColor = css_color; }

    // ----- IDomElement Interface -----

    /// Mounts this image into a parent WebLayout container with the given alignment.
    void WebImage::MountToLayout(WebLayout& parent, Alignment align) { parent.AddElement(this, align); }

    /// Re-applies all tracked properties to the underlying DOM element.
    void WebImage::SyncFromModel() {
        if (!EnsureElementAvailable(mElement, mId, "SyncFromModel"))
            return;
    ...
    void WebImage::Draw(WebCanvas& canvas) {
        if (mHasError && mErrorMode == ImageErrorMode::BlankRect) {
            const auto resolve_draw_size = [](float canvas_size, int dom_size) {
                return canvas_size > 0.0f ? canvas_size
    ...
    void WebImage::HandleError() {
        mHasError = true;
        mIsLoaded = false;
        std::cerr << "WebImage error: failed to load image '" << mSrc << "' (id=" << mId << ")" << std::endl;
    ...
    void WebImage::AttachListeners() {
        if (!EnsureElementAvailable(mElement, mId, "AttachListeners"))
            return;
        ...
        EM_ASM(
                {
                    ...
                    var registryId = $1;
                    el.addEventListener("load", function() { Module._WebImage_handleLoad(registryId); });
                    el.addEventListener("error", function() { Module._WebImage_handleError(registryId); });
                },
    ...
    void WebImage::ApplyPlaceholder() {
        if (!EnsureElementAvailable(mElement, mId, "ApplyPlaceholder"))
            return;
...
EMSCRIPTEN_KEEPALIVE
int WebImage_registryContains(int registry_id) { return cse498::WebImage::LookupImage(registry_id) != nullptr ? 1 : 0; }
}
