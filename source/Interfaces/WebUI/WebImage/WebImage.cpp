...
// Registry maps integer IDs to WebImage pointers, avoiding unsafe
// pointer-to-int casts when forwarding events from JavaScript back to C++.
...
void UnregisterImage(int id) { sImageRegistry.erase(id); }
...
/// Removes element from DOM, deregisters from the event registry, and nulls the
/// element.
...
/// Constructs a WebImage, creates the <img> DOM element, and attaches event
/// listeners.
WebImage::WebImage(const std::string &src, const std::string &alt_text)
    : mSrc(src), mAltText(alt_text), mElement(val::null()) {
...
WebImage::WebImage(WebImage &&other) noexcept
    : mSrc(std::move(other.mSrc)), mAltText(std::move(other.mAltText)),
      mWidth(other.mWidth), mHeight(other.mHeight), mOpacity(other.mOpacity),
      mIsVisible(other.mIsVisible), mIsLoaded(other.mIsLoaded),
      mHasError(other.mHasError), mErrorMode(other.mErrorMode),
      ...
      mElement(other.mElement), mId(std::move(other.mId)),
...
void WebImage::Resize(int width_px, int height_px, bool maintain_aspect_ratio) {
  ...
  if (!mElement.isNull()) {
    ...
    mElement["style"].set("height", ToPx(height_px));
    mElement["style"].set(
        "objectFit", std::string(maintain_aspect_ratio ? "contain" : "fill"));
  }
}
...
void WebImage::SetOpacity(double alpha) {
  assert(alpha >= 0.0 && alpha <= 1.0 &&
         "SetOpacity: alpha must be in [0.0, 1.0]");
  mOpacity = alpha;
  if (!mElement.isNull()) {
...
/// Mounts this image into a parent WebLayout container with the given
/// alignment.
...
void WebImage::Unmount() { RemoveFromDom(); }
...
const std::string &WebImage::Id() const { return mId; }

// ----- ICanvasElement Interface -----

/// Draws the image on a WebCanvas (stub; primary rendering is DOM-based).
void WebImage::Draw(WebCanvas &canvas) { (void)canvas; }

// ----- Event Handlers -----

/// Called when the image finishes loading; sets state and fires the load
/// callback.
...
/// Called when the image fails to load; applies placeholder if configured and
...
/// Attaches JS load/error event listeners that forward to
/// HandleLoad/HandleError via registry ID.
  ...
  EM_ASM(
      {
        ...
        var registryId = $1;
        el.addEventListener(
            "load", function() { Module._WebImage_handleLoad(registryId); });
        el.addEventListener(
            "error", function() { Module._WebImage_handleError(registryId); });
      },
      mElement.as_handle(), regId);
}

/// Replaces the broken image with a colored rectangle of the configured
/// placeholder color.
