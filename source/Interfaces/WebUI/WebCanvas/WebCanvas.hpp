...
/**
 * WebCanvas
 * - As an IDomElement: can be mounted/unmounted/synced by WebLayout (DOM
 * lifecycle).
 * - As a canvas manager: owns ICanvasElement objects and dispatches Draw() in
 * RenderFrame().
 *
...
public:
  ...
  // - In Emscripten builds these issue Canvas2D calls on the <canvas
  // id="Id()">. void Clear(const std::string& cssColor = "#000000");
