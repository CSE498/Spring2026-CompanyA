...
namespace cse498 
{
  ...
  inline val GetElement(const string & Id) {
  ...
  inline val GetProperty(val element, const string & property) {
    if (element.isNull() || element.isUndefined()) return val::undefined();
  ...
  if (element.isNull() || element.isUndefined()) return "";

  val style_attr = element.call<val>("getAttribute", string("style"));
  if (!style_attr.isString()) return "";
  ...
  while ((start = style_str.find(search_key, start)) != string::npos) {
    ...
    string value = (end == string::npos) 
                   ? style_str.substr(start) 
                   : style_str.substr(start, end - start);
