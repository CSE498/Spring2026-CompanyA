...
public:
  ...
  void Notify(const std::string &message,
    ...
    std::cout << "[Agent Notification] type= " << msg_type
...
bool Game::Initialize() {
  ...
  auto LoadCheck = [&](const std::string &name,
  ...
  if (!LoadCheck("grass", std::string(ASSETS_DIR) + "Tiles/grass.png"))
    return false;
  if (!LoadCheck("grass_flowers",
                 std::string(ASSETS_DIR) + "Tiles/grass_flowers.png"))
    return false;
  if (!LoadCheck("grass_bones",
                 std::string(ASSETS_DIR) + "Tiles/grass_bones.png"))
    return false;
  if (!LoadCheck("grass_mud", std::string(ASSETS_DIR) + "Tiles/grass_mud.png"))
    return false;
  if (!LoadCheck("grass_rock",
                 std::string(ASSETS_DIR) + "Tiles/grass_rock.png"))
    return false;

  // Structure
  if (!LoadCheck("entrance",
                 std::string(ASSETS_DIR) + "Tiles/grass_left_entrance.png"))
    return false;

  // Border walls
  if (!LoadCheck("wall_left",
                 std::string(ASSETS_DIR) + "Tiles/grass_wall_left.png"))
    return false;
  if (!LoadCheck("wall_right",
                 std::string(ASSETS_DIR) + "Tiles/grass_wall_right.png"))
    return false;
  if (!LoadCheck("wall_top",
                 std::string(ASSETS_DIR) + "Tiles/grass_wall_up.png"))
    return false;
  if (!LoadCheck("wall_bottom",
                 std::string(ASSETS_DIR) + "Tiles/grass_wall_bottom.png"))
    return false;
  if (!LoadCheck("wall_corner",
                 std::string(ASSETS_DIR) + "Tiles/grass_wall_up.png"))
    return false;
  ...
  if (!LoadCheck("lumber_yard",
                 std::string(ASSETS_DIR) + "Tiles/lumber_yard.png"))
    return false;
...
void Game::SetupOverworld() {
  ...
  int max_cam_x =
      std::max(0, static_cast<int>(mOverworldGrid->GetWidth()) - tiles_x);
  int max_cam_y =
      std::max(0, static_cast<int>(mOverworldGrid->GetHeight()) - tiles_y);
  mCamX = std::clamp(static_cast<int>(player_pos.CellX()) - tiles_x / 2, 0,
                     max_cam_x);
  mCamY = std::clamp(static_cast<int>(player_pos.CellY()) - tiles_y / 2, 0,
                     max_cam_y);
}
...
void Game::SetupMainMenu() {
  ...
  mMainMenu.AddOption("New Game",
                      [this]() { TransitionTo(GameState::OVERWORLD); });

  mMainMenu.AddOption("Settings",
...
void Game::SetupPauseMenu() {
  ...
  mPauseMenu.AddOption("Settings",
                       [this]() { TransitionTo(GameState::SETTINGS); });

  mPauseMenu.AddOption("Quit to Main Menu",
  ...
  while (SDL_PollEvent(&event)) {
    ...
    if (event.type == SDL_KEYDOWN) {
      ...
      case SDLK_RETURN:
        ...
        // Player movement and interaction — one turn per keypress with 150ms
        // cooldown
      case SDLK_w:
      case SDLK_s:
      case SDLK_a:
...
void Game::RenderOverworld() {
  ...
  for (size_t i = 0; i < mInteractiveWorld->GetNumAgents(); ++i) {
    ...
    mImageManager->DrawImage(mInteractiveWorld->GetAgentSpriteName(), screen_x,
                             screen_y, tw, th);
  }
  ...
  mImageManager->DrawImage(mInteractiveWorld->GetPlayerSpriteName(),
                           player_screen_x, player_screen_y, tw, th);
...
void Game::RenderWorld(const ImageGrid &grid, int camX, int camY) {
  grid.DrawViewport(*mImageManager, camX, camY, mGameView->GetWidth(),
                    mGameView->GetHeight());
}
...
void Game::ProcessPlayerMove(SDL_Keycode key) {
  if (mState == GameState::OVERWORLD) {
    ...
    int max_cam_x =
        std::max(0, static_cast<int>(mDungeonGrid->GetWidth()) - Tiles_x);
    int max_cam_y =
        std::max(0, static_cast<int>(mDungeonGrid->GetHeight()) - Tiles_y);
