...
/**
 * Destructor (renderer should not be destroyed in this class)
 */
ImageManager::~ImageManager()
{
    // iterate through map & destroy textures (value in the key-value pair)
    for (auto& pair : mTextures)
    {
        SDL_DestroyTexture(pair.second);
    }
    // remove entire pairs after memory is destroyed
    mTextures.clear();
}
