...
public:
    ...
    void GUIError(const std::string& message) const {
        ...
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_WARNING, "Game Error", message.c_str(), mWindow);
    }

    /// Logs an error message and continues execution.
    void LogError(const std::string& message) const { PrintInColor(mLogErrorColor, "[LOG ERROR] " + message); }

    /// Logs a warning message and continues execution.
    void LogWarning(const std::string& message) const { PrintInColor(mLogWarningColor, "[LOG WARN] " + message); }
};
