...
namespace cse498 {
    ...
    void AIWorld::PrintWorldState(const std::string &header) const {
        ...
        for (size_t a = 0; a < GetNumAgents(); ++a) {
            ...
            std::cout << "Agent " << agent.GetName() << " | HP: " << st.mHP << "/" << st.mMaxHP
                      << " | ATK: " << st.mAttack << " | HEAL: " << st.mHealCharges << " | SCORE: " << st.mScore
                      << '\n';
        }
        for (const auto &enemy: mEnemies) {
    ...
    void AIWorld::HandleEnemyTurn(EnemyState &enemy) {
        ...
        for (size_t i = 0; i < GetNumAgents(); ++i) {
            ...
            const int dist = static_cast<int>(
                    std::abs(static_cast<int>(enemy.mPosition.CellX()) - static_cast<int>(pos.CellX())) +
    ...
    void AIWorld::UpdateWorld() {
        ...
        const bool any_agent_alive = std::any_of(mAgentState.begin(), mAgentState.end(),
