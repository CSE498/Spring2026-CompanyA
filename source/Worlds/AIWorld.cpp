...
LootItem::LootItem(size_t id, const std::string &name, const WorldBase &world,
                   ItemKind kind, int power)
...
void AIWorld::PrintWorldState(const std::string &header) const {
    ...
    for (size_t x = 0; x < main_grid.GetWidth(); ++x) {
      ...
      for (const auto &item_ptr : item_set) {
        ...
        if (!loot_item || loot_item->IsConsumed() ||
            !loot_item->GetLocation().IsPosition())
          continue;
        if (loot_item->GetLocation().AsWorldPosition() == pos) {
          symbol =
              (loot_item->GetKind() == LootItem::ItemKind::Weapon) ? 'W' : 'H';
        }
      }
  ...
  for (size_t a = 0; a < GetNumAgents(); ++a) {
    ...
    std::cout << "Agent " << agent.GetName() << " | HP: " << st.mHP << "/"
              << st.mMaxHP << " | ATK: " << st.mAttack
              << " | HEAL: " << st.mHealCharges << " | SCORE: " << st.mScore
              << '\n';
  }
  for (const auto &enemy : mEnemies) {
    std::cout << "Enemy#" << enemy.mID << " | HP: " << enemy.mHP << " | Pos: ("
              << enemy.mPosition.CellX() << "," << enemy.mPosition.CellY()
              << ")"
...
void AIWorld::HandleEnemyTurn(EnemyState &enemy) {
  ...
  for (size_t i = 0; i < GetNumAgents(); ++i) {
    ...
    const int dist =
        static_cast<int>(std::abs(static_cast<int>(enemy.mPosition.CellX()) -
                                  static_cast<int>(pos.CellX())) +
                         std::abs(static_cast<int>(enemy.mPosition.CellY()) -
                                  static_cast<int>(pos.CellY())));
    if (dist < best_dist) {
      best_dist = dist;
  ...
  if (dist == 1) {
    ...
    if (mVerboseMode) {
      std::cout << "[Enemy " << enemy.mID << "] attacks " << target.GetName()
                << " for " << enemy.mAttack << " damage.\n";
    }
    if (state.mHP <= 0) {
      state.mHP = 0;
      run_over = true;
  ...
  for (const WorldPosition &next : moves) {
    ...
    if (mVerboseMode) {
      std::cout << "[Enemy " << enemy.mID << "] moves to ("
                << enemy.mPosition.CellX() << "," << enemy.mPosition.CellY()
                << ").\n";
    }
    return;
...
AIWorld::AIWorld() : MazeWorld() {
  auto add_loot = [&](const std::string &name, LootItem::ItemKind kind,
                      int power, WorldPosition pos) {
    auto item =
        std::make_unique<LootItem>(item_set.size(), name, *this, kind, power);
...
std::vector<AIWorld::VisibleItem> AIWorld::GetFloorItems() const {
  ...
  for (const auto &item_ptr : item_set) {
    ...
    if (!loot_item->GetLocation().IsPosition())
      continue;
    visible.push_back(VisibleItem{loot_item->GetID(),
                                  loot_item->GetLocation().AsWorldPosition(),
...
int AIWorld::DoAction(AgentBase &agent, size_t action_id) {
  ...
  if (action_id == PICKUP_ITEM) {
    ...
    if (loot_item->GetKind() == LootItem::ItemKind::Weapon) {
      ...
      state.mScore += 15;
      if (mVerboseMode) {
        std::cout << "[" << agent.GetName() << "] picked up weapon '"
                  << loot_item->GetName() << "' (+" << loot_item->GetPower()
                  << " attack).\n";
      }
    } else {
      ...
      state.mScore += 10;
      if (mVerboseMode) {
        std::cout << "[" << agent.GetName() << "] picked up heal item '"
                  << loot_item->GetName() << "' (+" << loot_item->GetPower()
                  << " heal charges).\n";
      }
    }
  ...
  if (action_id >= ATTACK_UP && action_id <= ATTACK_RIGHT) {
    ...
    if (mVerboseMode) {
      std::cout << "[" << agent.GetName() << "] attacks Enemy#" << enemy.mID
                << " for " << state.mAttack << " damage.\n";
    }
    if (enemy.mHP <= 0) {
      enemy.mHP = 0;
      enemy.mAlive = false;
...
void AIWorld::RunAgents() {
  for (const auto &agent_ptr : agent_set) {
    ...
    if (mVerboseMode) {
      std::cout << "[Turn " << mCurrentTurn << "] " << agent.GetName() << " -> "
...
void AIWorld::UpdateWorld() {
  ...
  const bool any_agent_alive =
      std::any_of(mAgentState.begin(), mAgentState.end(),
                  [](const auto &entry) { return entry.second.mHP > 0; });
  const bool any_enemy_alive =
      std::any_of(mEnemies.begin(), mEnemies.end(),
                  [](const EnemyState &enemy) { return enemy.mAlive; });
