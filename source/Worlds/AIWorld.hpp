...
public:
  ...
  LootItem(size_t id, const std::string &name, const WorldBase &world,
...
protected:
  ...
  [[nodiscard]] bool IsEnemyAt(WorldPosition pos,
                               size_t *enemy_index = nullptr) const;
  [[nodiscard]] bool
  IsAgentAt(WorldPosition pos,
            size_t skip_agent_id = static_cast<size_t>(-1)) const;
...
public:
  ...
  [[nodiscard]] const std::vector<EnemyState> &GetEnemies() const {
    return mEnemies;
  }
  [[nodiscard]] const AgentState &GetAgentState(size_t id) const {
    return mAgentState.at(id);
  }
