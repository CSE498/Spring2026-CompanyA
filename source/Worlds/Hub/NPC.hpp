...
class NPC {
private:
    std::shared_ptr<Building> m_building{}; ///< Building associated with this NPC.
...
public:
     ...
     */
    explicit NPC(std::shared_ptr<Building> building) : m_building(std::move(building)) {
    ...
    std::string GetUpgradeUI() const {
        ...
        output << m_building->GetName() << " - level: " << m_building->GetCurrentLevel();

        if (const auto next_upgrade = m_building->GetNextUpgradeInfo(); next_upgrade.has_value()) {
            output << " | next upgrade: " << next_upgrade->quantity << ' ' << ItemTypeToString(next_upgrade->item)
                   << " for level " << m_building->GetNextUpgradeLevel();
        } else {
     ...
     */
    void ShowUpgradeUI(std::ostream& output = std::cout) const { output << GetUpgradeUI() << '\n'; }
     ...
     */
    [[nodiscard]]
    std::expected<void, Building::UpgradeRejectionType> AttemptUpgrade(InteractiveWorldInventory& inventory) {
        ...
        assert(next_upgrade.has_value() && "Non-max-level buildings must expose a next upgrade");

        int available_quantity = inventory.GetAmount(next_upgrade->item);

        auto upgrade_result = m_building->Upgrade(next_upgrade->item, available_quantity);

        if (!upgrade_result) {
            return std::unexpected(upgrade_result.error());
        }

        const bool removed = inventory.RemoveItem(next_upgrade->item, next_upgrade->quantity);
