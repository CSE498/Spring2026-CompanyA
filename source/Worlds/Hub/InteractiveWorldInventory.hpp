...
public:
     ...
     */
    [[nodiscard]] bool HasEnough(ItemType item, ItemCount amount) const { return GetAmount(item) >= amount; }

    /**
     * @brief Get a read-only view of all stored inventory data.
     * @return A const reference to the internal item map.
     */
    [[nodiscard]] const std::map<ItemType, ItemCount>& GetItems() const { return m_items; }
