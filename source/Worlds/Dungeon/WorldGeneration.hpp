...
/// @brief utilizes BSP-Tree.hpp and RoomHolder.hpp to generate a dungeon level, represented as a vector of strings to
/// be loaded in DungeonBase.
...
public:
    ...
    WorldGeneration(const LevelBase& level) :
        m_bsp(level), // For now, the constructor for BSP_tree room creation is going to generate rooms immediately
                      // when initialized, will reformat as level specifications become more detailed
        mRoomHolder(level), m_grid(m_bsp.GetHeight(), std::string(m_bsp.GetWidth(), '#')) {}
    ...
    /// @return
    [[nodiscard]] BSP GetBSP() const { return m_bsp; }
    ...
    [[nodiscard]] std::vector<LinkedRooms> GetConnectedRooms() const { return m_connected_rooms; }

    [[nodiscard]] const std::string GetImageFile(char c) { return mRoomHolder.GetImageFile(c); };


private:
    /// @brief takes the leaf node's (x,y) coordinates and room information from the BSP_Tree and translates it onto
    /// mGrid
    ...
    Point PostOrderRoomConnect(BSPNode node) {
        // Need to change Node node to idx int later
