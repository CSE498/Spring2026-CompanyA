...
private:
    ...
    std::unique_ptr<LevelBase> m_level =
            std::make_unique<ForestLevel>(); // The currently pointed to level that the player agent is on
...
protected:
    ...
    WeightedSet<std::string> m_item_pool; // A pool of possible items to generate
...
public:
    DungeonWorld() : m_generation(*m_level) {
        ...
        m_internal_obstacle = main_grid.AddCellType("wall", "internal obstacle wall.", '$');
    ...
    /// Debug tool used for updating level progression + progressing dungeon levels (dungeonone, dungeontwo,
    /// dungeonthree)
    void UserInput() {
    ...
    int DoAction(AgentBase& agent, size_t action_id) override {
        ...
        if (main_grid[new_position] == m_wall_id || main_grid[new_position] == m_internal_obstacle ||
            main_grid[new_position] == m_upper_external || main_grid[new_position] == m_lower_external ||
            main_grid[new_position] == m_left_external || main_grid[new_position] == m_right_external) {
            return false;
        }
    ...
    [[nodiscard]] const std::string GetImageFile(char c) { return m_generation.GetImageFile(c); };
};
