...
public:
    ...
    std::vector<ObjectId> Query(const std::vector<std::string_view>& includeTags,
...
private:
    ...
    std::vector<std::reference_wrapper<const ObjectSet>>
    CollectExistingSets(const std::vector<std::string_view>& tags) const;
    std::optional<std::reference_wrapper<const ObjectSet>>
    FindSmallestIncludeSet(const std::vector<std::string_view>& includeTags) const;
    ...
    std::vector<ObjectId>
    QueryFromAllObjects(const std::vector<std::reference_wrapper<const ObjectSet>>& excludeSets) const;
    std::vector<ObjectId>
    QueryFromBaseSet(const ObjectSet& baseSet, const std::vector<std::string_view>& includeTags,
                     const std::vector<std::reference_wrapper<const ObjectSet>>& excludeSets) const;

    static bool IsValid(const std::expected<void, FindSetError>& result) { return result.has_value(); }

    // Used ChatGPT to help with string_view issue without it having to create a  temporary string to find tags
    // (redundant) This enables transparent hash and equality (below), which means no allocation on lookup
    ...
    struct TransparentStringEqual {
        ...
        constexpr bool operator()(const std::string& a, std::string_view b) const noexcept {
            return std::string_view(a) == b;
        }
        constexpr bool operator()(std::string_view a, const std::string& b) const noexcept {
            return a == std::string_view(b);
        }
    };

    // TagMap now incorporates the Transparent Hash + Equality for no allocation lookup
    using TagMap = std::unordered_map<std::string, ObjectSet, TransparentStringHash, TransparentStringEqual>;
