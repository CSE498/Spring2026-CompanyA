...
private:
  ...
  // Used ChatGPT to help with string_view issue without it having to create a
  // temporary string to find tags (redundant) This enables transparent hash and
  // equality (below), which means no allocation on lookup
  ...
  // TagMap now incorporates the Transparent Hash + Equality for no allocation
  // lookup
  using TagMap =
      std::unordered_map<std::string, ObjectSet, TransparentStringHash,
                         TransparentStringEqual>;

  std::expected<const ObjectSet *, FindSetError>
  FindSet(std::string_view tag) const;
