...
struct CellTypeModifiers {
  ...
  CellTypeModifiers(bool breakable, bool locked, bool traversable)
      : isBreakable(breakable), locked(locked), traversable(traversable) {}
};
...
struct CellType {
  ...
  CellTypeModifiers
      tile_modifiers; ///< struct object to determine modifiers of the cell
...
public:
  ...
  WorldGrid()
      : width(UNKNOWN_CELL_TYPE_ID), height(UNKNOWN_CELL_TYPE_ID), cells() {
    AddCellType("Unknown",
                "This is an invalid cell type and should not be reachable.");
  }
  ...
  size_t AddCellType(const std::string &name, const std::string &desc = "",
                     char symbol = '\0', CellTypeModifiers mod = {}) {
