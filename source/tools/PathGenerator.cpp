...
namespace cse498 {
bool PathGenerator::IsTravelable(const WorldPosition &from,
                                 const PathVector &dir,
                                 const PathRequest &request) {
  auto next_pos = from + dir;
  if (!request.mWorldGrid.IsWalkable(next_pos) ||
      request.mAvoidTiles.contains(round(next_pos)))
    return false;
  if (dir.X() == 0 ||
      dir.Y() ==
          0) // You can always move in the 4 cardinals given tile is valid
    return true;
  // capable of being traveled within only this context. DOES NOT determine if
  // walkable. This is just if it can be walked to. Example:
  /*      x_x
   *      x__
   *      _x_
   *      in this 3x3 the bottom left can't be traveled to because two blocks
   * are cutting it off
   */
  const WorldPosition test1 = from + dir.mult({1, 0});
  const WorldPosition test2 = from + dir.mult({0, 1});
  return request.mWorldGrid.IsWalkable(test1) &&
         request.mWorldGrid.IsWalkable(test2);
}

PathGenerator::CircleTravel
PathGenerator::IsTravelableCircle(const WorldPosition &from,
                                  const WorldPosition &to,
                                  const PathRequest &request) {
  // find the direction of the 'to'
  const auto vec = PathVector(from, to);
  if (vec.getMagnitude() > 1 + 1e-6) {
    // Some point was skipped. We no longer know if we can travel there
    // directly: Find weird path there instead
    auto pth = AStarSearch(from, request, SingleGoalHeuristic{to},
                           vec.getMagnitude() * 2, false);
    if (!pth.empty()) {
      // it brings us to the right tile but we change the actual position on
      // that tile to what we want. This will be generally better though some
      // spots that are longer but if we can get to the tile then we can get to
      // a specific position on that tile type of thing.
      // TODO: warning if the agent has hitbox > 0 then this may push agent into
      // the wall to get to this position
      pth.pop_back();
      pth.push_back(to);
...
bool PathGenerator::IsPointBefore(const WorldPosition &test_pt,
  ...
  // Again clockwise = Counterclockwise and vise versa. Everything is opposite
  // and it hurts my head Assume both points are on the circle
  const PathVector center_to_test(test_pt, center);
  const PathVector center_to_relative(relative_pt, center);
  assert(std::abs(center_to_relative.getMagnitude() -
                  center_to_test.getMagnitude()) < 1e-6);

  const PathVector test(test_pt.X() - center.X(), test_pt.Y() - center.Y());
  const PathVector relative(relative_pt.X() - center.X(),
                            relative_pt.Y() - center.Y());

  // This determines if a point is before another point by comparing the angles
  // they make. The weird if statements are because of how the angle is returned
  // from getAngle -- it returns in domain [-pi, pi]
...
PathGenerator::AStarReconstruction(const std::shared_ptr<ANode> &node) {
...
WorldPosition PathGenerator::FindNextCirclePos(const WorldPosition &start,
  ...
  assert(std::abs(center_to_start.getMagnitude() - radius) <
         EP); // just to ensure no bugs from this.

  // NOTE!! opposite from expected because Worlds are inverted
  const double direction_decider = flag == CircleDirectionFlag::CW ? 1 : -1;

  // Find rotation angle: (just math equation of triangle bisect. Not magic
  // numbers)
...
WorldPath PathGenerator::MakeCircle(const WorldPosition &start,
  ...
  // Now we should be good to make the circle from here and take whatever
  // strategy to make it.
  result.push_back(start);

  if (flag == PathFlag::Skip) {
    WorldPosition next =
        FindNextCirclePos(start, circ_center, circ_radius, circle_flag);
    ...
    // add the last point provided that 1. it is a valid tile and 2. that it
    // comes before the start of the circle
    ...
    if (possible) {
      ...
      if (IsPointBefore(next, start, circ_center, circle_flag) &&
          EuclideanDistance(result.back(), start) >
              STEP_SIZE * STEP_CIRCLE_TOLERANCE)
        result.push_back(next);
    } else {
      // Tile invalid just go to the start - didn't do this originally since it
      // is slightly different in cases that matter
      auto [possible2, path2] =
          IsTravelableCircle(result.back(), start, request);
      if (possible2)
  ...
  if (flag ==
      PathFlag::Expand) // ensures this is the case if other cases are added.
  {
    // this case is strange and needs to be updated IMMEDIATELY once physics
    // changes and more information exists
    // TODO: Update this once physics and more information about positions
    // exists, size of player, etc.
    // TODO: Expand is supposed to hug walls and corners but is impossible
    // without size of player/collision test
    // This case is also really slow but I think it is useful for calculating
    // things and saving them in jsons
    ...
    auto inner = [&] {
      ...
      if (possible)
        result.push_back(next); // We do not extend paths. Just need to know it
                                // is possible or not
      else {
        const double search_distance =
            PathVector(result.back(), next).getMagnitude() *
            CIRCLE_EXPAND_MULTIPLIER;
        auto path =
            AStarSearch(result.back(), request, SingleGoalHeuristic{next},
                        search_distance, true);
        if (path.size() > 1)
          result.insert(result.end(), path.begin() + 1,
                        path.end()); // skip first pos for repeats
      }
    ...
    // This case is different from the previous ending connection because in
    // this case we are always technically very close to the starting position
    // so we only worry if it is a missing point to fill the gap
    if (IsPointBefore(next, start, circ_center, circle_flag) &&
        EuclideanDistance(result.back(), start) > 1) {
      const size_t size_temp = result.size();
      inner(); // call it one more time with that position
      if (size_temp != result.size())
        return WorldPath(
            result); // point increased it is close enough and can loop
      // ELSE:
      // Nothing was added. Try to connect to the start point instead:
      auto [ending2_possible, ending2_path] =
          IsTravelableCircle(result.back(), start, request);
      if (ending2_possible)
        result.insert(result.end(), ending2_path.begin(),
                      ending2_path.end() - 1);
    }
...
std::optional<CirclePath> PathGenerator::FindCircularPath(
    const WorldPosition &agent_pos, const WorldPosition &circ_center,
    const double circ_radius, const PathRequest &request, const PathFlag flag,
    const CircleDirectionFlag circle_flag) {
  /*
   * Goal is to return a list of points that are 1 unit apart for the most part
   * making a complete circle The start of the circle is at the point nearest to
   * the agent given that tile is reachable. Otherwise it is the top of the
   * circle. --> If top is unreachable then it will be the first point that
   * appears that is reachable from the top and going clockwise. If you want to
   * go counterclockwise then the list is just reversed -- that can be done
   * later.
   */
  ...
  // We need to encapsulate the process of finding the initial path to a point
  // on the circle before generating the circle It is two separate things and
  // shouldn't be combined because then it needs to be separated again. HOWEVER,
  // the processes are similar so there is a little bit of redundancy that is
  // bad to combine into 1 func
  auto [easy_travel_possible, easy_travel_path] =
      IsTravelableCircle(agent_pos, start_point, request);
  if (easy_travel_possible)
    path_to_circle.insert(path_to_circle.end(), easy_travel_path.begin(),
                          easy_travel_path.end());
  else {
    // Try to find start point
    WorldPosition next =
        FindNextCirclePos(start_point, circ_center, circ_radius, circle_flag);
  ...
  // We should have a valid path now to the start of the circle and a position
  // of the circle. Now we just need to know how to loop around the circle
  const auto circle_path = MakeCircle(start_point, circ_center, circ_radius,
                                      request, flag, circle_flag);

  // Need to fully write it out here.
  return CirclePath(WorldPath(path_to_circle), circle_path);
}

std::optional<std::vector<WorldPosition>> PathGenerator::MakeRectangleLoop(
    const WorldPosition &bot_left, const WorldPosition &top_right,
    const PathRequest &request, const CircleDirectionFlag flag) {
  // NOTE: does this return cell int positions or doubles? -- doubles if given
  // doubles
...
std::optional<CirclePath> PathGenerator::FindRectangularLoopPath(
  ...
  // TODO MULTIGOAL H. You need to make map of INTS Cell positions for
  // comparison in calcs.
  ...
  // The original start_path worked with finding cell positions, not actual
  // positions. This can affect The creation of the loop vs intended.
...
PathGenerator::FindShortestPath(const WorldPosition &start,
                                const WorldPosition &end,
                                const PathRequest &request) {
  /*
   * We need to make the shortest path from start to end with certain other
   * parameters. We are going to do this a "bad" way of following 8 directions
   * around the player and do A* search on that result to the goal. This
   * probably won't be that quick and can be made faster after more information
   * about the games structure is understood, and I can work/edit the right
   * files.
   */
  // Step 1:
...
std::optional<WorldPath> PathGenerator::FindManhattanPath(
    const WorldPosition &start, const WorldPosition &end,
    const PathRequest &request, const CircleDirectionFlag flag) {
  if (!IsTravelable(
          start, {0, 0},
          request)) // Check start position. A little verbose but simple
    return {};
  ...
  // Test Case: check 2D plane given center is the start point and check every
  // point
}
} // namespace cse498
