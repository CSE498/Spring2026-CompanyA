...
bool OnSegment(const Vec2 &a, const Vec2 &b, const Vec2 &p) {
  ...
  return p.x >= minx - kEps && p.x <= maxx + kEps && p.y >= miny - kEps &&
         p.y <= maxy + kEps;
}

// Segment/segment intersection test.
bool SegmentsIntersect(const WorldPosition &a0, const WorldPosition &a1,
                       const WorldPosition &b0, const WorldPosition &b1) {
