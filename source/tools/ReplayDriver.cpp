...
// void cse498::ReplayDriver::ReplayByTimeRange(double start_time, double
// end_time)
// {
...
bool cse498::ReplayDriver::SaveReplayToFile(const std::string &filename) {
  if (IsActionLogSet() == true) {
    ...
    for (Action action : m_lastReplayedActions) {
      outFile << action.EntityId << "," << action.ActionType << ","
              << action.Timestamp << "," << action.Position.X() << ","
              << action.Position.Y() << "," << action.NewPosition.X() << ","
              << action.NewPosition.Y() << "," << action.SequenceNumber << "\n";
    }
    outFile.close();
    return true;
  }
