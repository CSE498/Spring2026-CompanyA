 ...
 * 1. Movement (You have a point a --> point b scaled by delta time and speed of
 * the character)
 * 2. Calculations for collisions (you can test with vectors if the distance
 * between two vectors are too close = collision)
 *
 */
class PathVector {
private:
...
public:
  ...
  /// Hadamard Product
  [[nodiscard]] PathVector mult(const PathVector &second) const {
    return {mX * second.mX, mY * second.mY};
  }
  [[nodiscard]] PathVector mult(const double x, const double y) const {
    return {mX * x, mY * y};
  }
   ...
   * @param scale_val scale factor for the vector. Changes x,y by this amount so
   * mag --> mag * scale_val
  ...
  /**
   * rotates the path vector with respect to its start position counterclockwise
   * by angle
  ...
  /**
   * Projects argument onto calling object returning new position of where the
   * projection lands
   * @param this_onto_that whatever is put here is projected onto the object
