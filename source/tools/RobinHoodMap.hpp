...
namespace cse498 {
  ...
  private:
    ...
    [[nodiscard]] size_type FindIndex(const Key & key) const {
      ...
      while (true) {
        if (!buckets_[idx].occupied)              return capacity_;
        if (buckets_[idx].probe_distance < dist)  return capacity_;
        if (key_eq_(buckets_[idx].kv.first, key)) return idx;
        ++dist;
        idx = (idx + 1) % capacity_;
      }
    }
  ...
  public:
    ...
    public:
      ...
      Iterator(Bucket * ptr, size_type index, size_type cap)
        : ptr_(ptr), index_(index), cap_(cap) { SkipEmpty(); }
    ...
    explicit RobinHoodMap(
        size_type initial_capacity  = DEFAULT_CAPACITY,
