 ...
 * @brief A high-performance hash map using open addressing with robin hood
 * hashing. Robin Hood hashing resolves collisions by stealing from "rich"
 * entries (close to their ideal slot) and giving to "poor" ones (far from
 * theirs), keeping probe distances short and lookup times even.
 * @note Status: PROPOSAL
 * @citation: Used Claude Code Opus 4.6 to help me understand the algorithm and
 * implement it.
 **/
...
/// @class RobinHoodMap
/// @brief High-performance hash map using open addressing with robin hood
/// hashing.
///
...
///
/// @tparam Key      Key type (must be hashable by Hash and comparable by
/// KeyEqual)
...
private:
  ...
  struct Bucket {
    ...
    size_type probe_distance =
        0;                 ///< Steps away from the ideal (hashed) position
  ...
  [[nodiscard]] size_type BucketIndex(const Key &key) const {
    return hash_fn_(key) % capacity_;
  }

  /// @brief Would inserting one more element exceed the load factor? also asks
  /// ' is the table too full? '
  [[nodiscard]] bool ShouldRehash() const {
    return static_cast<double>(size_ + 1) / static_cast<double>(capacity_) >
           max_load_factor_;
  }
...
public:
  ...
  public:
    ...
    Iterator() = default;
    Iterator(Bucket *ptr, size_type index, size_type cap)
        : ptr_(ptr), index_(index), cap_(cap) {
      SkipEmpty();
    }
  ...
  public:
    ...
    ConstIterator(const Bucket *ptr, size_type index, size_type cap)
        : ptr_(ptr), index_(index), cap_(cap) {
      SkipEmpty();
    }
  ...
  explicit RobinHoodMap(size_type initial_capacity = DEFAULT_CAPACITY,
                        double max_load_factor = DEFAULT_MAX_LOAD_FACTOR,
                        const Hash &hash_fn = Hash{},
                        const KeyEqual &key_eq = KeyEqual{})
      : buckets_(initial_capacity > 0 ? initial_capacity : 1, Bucket{}),
        size_(0), capacity_(initial_capacity > 0 ? initial_capacity : 1),
        max_load_factor_(max_load_factor), hash_fn_(hash_fn), key_eq_(key_eq) {
  ...
  RobinHoodMap(std::initializer_list<value_type> init,
               const Hash &hash_fn = Hash{},
               const KeyEqual &key_eq = KeyEqual{})
      : RobinHoodMap(DEFAULT_CAPACITY, DEFAULT_MAX_LOAD_FACTOR, hash_fn,
                     key_eq) {
    Reserve(init.size());
  ...
  void Reserve(size_type count) {
    size_type needed =
        static_cast<size_type>(static_cast<double>(count) / max_load_factor_) +
  ...
  [[nodiscard]] const_iterator begin() const {
    return ConstIterator(buckets_.data(), 0, capacity_);
  }

  [[nodiscard]] const_iterator end() const {
    return ConstIterator(buckets_.data(), capacity_, capacity_);
  }
