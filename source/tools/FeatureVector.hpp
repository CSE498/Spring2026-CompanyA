...
public:
  ...
  explicit FeatureVector(std::vector<T> values) : values_(std::move(values)) {}

  /// Construct from a raw pointer and length.
  /// @param data  Pointer to the first element.
  /// @param size  Number of elements to copy.
  FeatureVector(const T *data, std::size_t size) : values_(data, data + size) {}
  ...
  //     {
  //         throw std::invalid_argument("FeatureVector::operator+ vectors must
  //         be the same size");
  //     }
  ...
  [[nodiscard]] FeatureVector hadamard(const FeatureVector &other) const {
    if (values_.size() != other.values_.size()) {
      throw std::invalid_argument(
          "FeatureVector::hadamard vectors must be the same size");
    }
