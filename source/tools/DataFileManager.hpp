 ...
 *
 * This link helped me to understand how to trigger functions to build a new row
 * in the file:
 * https://stackoverflow.com/questions/67628186/how-to-write-the-result-of-a-function-to-a-file-c
 *
 * The function data_to_string(const T & data) used this link to help me
 * understand how to convert different data types to a string for storage in the
 * file: https://chatgpt.com/share/69925206-3bf4-8013-b807-859d6d7d1f89
 *
 * Learned and understand to check if two different types are convertible using
 * std::is_convertible_v from this link:
 * https://www.geeksforgeeks.org/cpp/stdis_convertible-template-in-c-with-examples/
 *
 * Learned a modern way of how to make a string constant and view only using
 * std::string_view from this link:
...
// The DataFileManager class is responsible for managing the storage of world
// data into a file.
class DataFileManager {
private:
  // The filename where the data will be stored.
  // This is initialized in the constructor and can be retrieved using the
  // GetFilename() method.
  std::string m_filename;

  // WorldBase &m_world; made it as a reference to guarantee that it is not
  // null.
  WorldBase &m_world; // This will be a unique_ptr stored in the world file once
                      // that file is complete

  template <typename T>

  // Helper function to convert various data types to a string representation
  // for file storage.
  std::string data_to_string(const T &data) const {
    if constexpr (std::is_convertible_v<T, std::string>) {
      ...
      throw std::runtime_error(
          "cse498::DataFileManager::data_to_string(): Unconvertible data type");
    }
  }

public:
  // Constructor to initialize the DataFileManager with a filename and a
  // reference to the world.
  DataFileManager(const std::string &filename, WorldBase *world)
      : m_filename(filename), m_world(*world) {
    if (filename.empty()) {
      throw std::runtime_error(
          "cse498::DataFileManager::Constructor: Filename cannot be empty");
    }
    if (world == nullptr) {
      throw std::runtime_error(
          "cse498::DataFileManager::Constructor: World pointer cannot be null");
    }
  }

  // Getter for the filename, allowing retrieval of the current filename being
  // used by the DataFileManager.
  std::string GetFilename() const { return m_filename; }

  template <typename T>

  // Function to store data in a specific format, taking an ID, a type, and the
  // data itself.
  std::string FormatData(int id, std::string_view type, const T &data) const {
    ...
    if (type != TILE_TYPE && type != AGENT_TYPE && type != BACKGROUND_TYPE) {
      throw std::runtime_error(
          "cse498::DataFileManager::FormatData(): Must provide a valid type "
          "[Tile, Agent, Background]");
    }
    data_stored += type;
  ...
  // Function to update the data file with the current state of the world,
  // including tile and agent information.
  bool Update() {
    std::ofstream file;
    ...
    std::string agents =
        FormatData(AGENT_ID, AGENT_TYPE, m_world.GetKnownAgents(agent));
