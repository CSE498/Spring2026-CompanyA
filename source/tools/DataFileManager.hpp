  ...
  public:
    ...
    void LoadData()
    {
      ...
      if (last_valid_line.empty()) {
        throw std::runtime_error("ERROR::cse498::DataFileManager::LoadData(): No JSON snapshot found in file " + m_filename);
      }
      ...
      std::vector<std::string> rows;
      {
        ...
        while (true) {
          ...
          if (!ParseQuotedString(last_valid_line, pos, row)) {
            throw std::runtime_error("cse498::DataFileManager::LoadData(): Invalid row string in 'tiles' array");
          }
          rows.push_back(row);
