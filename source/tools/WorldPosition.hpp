...
public:
  ...
  [[nodiscard]] Orientation Dir() const {
    return dir;
  } // Returns the current facing direction
  void SetDir(Orientation new_dir) {
    dir = new_dir;
  } // Sets a new facing direction
  ...
  [[nodiscard]] bool IsAdjacentCell(const WorldPosition &other) const {
    size_t dx =
        std::max(CellX(), other.CellX()) - std::min(CellX(), other.CellX());
    size_t dy =
        std::max(CellY(), other.CellY()) - std::min(CellY(), other.CellY());
    return (dx + dy == 1);
  }
