...
private:
  ...
  struct ActionEntry {
    std::type_index signature{
        typeid(void)}; // exact std::function signature we registered with
    std::function<std::any(const std::vector<std::any> &)>
        invoker;                            // type-erased call wrapper
...
public:
  ...
  bool AddFunction(std::string name, std::function<void()> func,
    ...
    return AddFunction<void>(std::move(name), std::move(func),
  ...
  std::enable_if_t<std::is_void_v<R>, bool> Trigger(std::string_view name,
                                                    Args &&...args) const {
   ...
   *
   * Note: Unordered maps do not guarantee order. If a stable order is needed
   * for UI, callers can sort the returned vector (or we can add a sorted
   * listing).
   */
   ...
   */
  bool SetDescription(std::string_view name,
  ...
  bool AddFunction(std::string name, std::function<R(Args...)> func,
    ...
    assert(static_cast<bool>(func) &&
           "ActionMap::AddFunction function must not be empty");
