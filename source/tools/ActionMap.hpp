...
class ActionMap {
private:
    enum class InvokeError { WrongArgCount, WrongArgType };
    ...
    struct ActionEntry {
        ...
        std::function<std::expected<std::any, InvokeError>(const std::vector<std::any>&)>
                mInvoker; // type-erased call wrapper
     ...
     */
    static bool IsValidName(std::string_view name) { return !name.empty(); }
     ...
     */
    auto Find(std::string_view name) { return mActions.find(std::string(name)); }
    ...
    MakeInvoker(std::function<R(Args...)> func) {
        return [fn = std::move(func)](const std::vector<std::any>& packedArgs) -> std::expected<std::any, InvokeError> {
    ...
    static std::expected<std::any, InvokeError> InvokeImpl(const std::function<R(Args...)>& func,
     ...
     * @return A pair containing an iterator to the action and a vector of packed arguments, or {mActions.end(), {}} if
     * validation or lookup fails
     */
...
public:
     ...
     */
    bool Trigger(std::string_view name) const { return TriggerVoid<>(name); }
    ...
    std::enable_if_t<std::is_void_v<R>, bool> Trigger(std::string_view name, Args&&... args) const {
     ...
     */
    template<typename R, typename... Args>
    std::enable_if_t<!std::is_void_v<R>, std::optional<R>> Trigger(std::string_view name, Args&&... args) const {
     ...
     * Note: This is not strictly necessary since Trigger* can be used to check existence, but it can be more efficient
     * if caller only needs to check presence without triggering.
     *
     ...
     */
    template<typename R, typename... Args>
    bool AddFunction(std::string name, std::function<R(Args...)> func,
                     std::optional<std::string> description = std::nullopt) {
