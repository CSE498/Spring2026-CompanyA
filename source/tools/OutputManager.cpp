 ...
 */
void cse498::OutputManager::Log(LogLevel level, LogCategory category,
...

  {
    std::scoped_lock lock(m_mutex);
    if (m_min == LogLevel::Silent ||
        static_cast<uint8_t>(level) < static_cast<uint8_t>(m_min)) {
      return;
    }
  ...
  auto now = std::chrono::steady_clock::now();
  auto elapsed =
      std::chrono::duration_cast<std::chrono::milliseconds>(now - start_copy)
          .count(); // Calculate elapsed time for timestamp
 ...
 */
std::string_view
cse498::OutputManager::CategoryName(LogCategory category) const {
  switch (category) {
