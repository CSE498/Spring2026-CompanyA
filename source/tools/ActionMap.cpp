...
std::size_t ActionMap::Size() const { return mActions.size(); }

void ActionMap::Clear() { mActions.clear(); }
...
ActionMap::GetDescription(std::string_view name) const {
  if (!IsValidName(name)) {
    return std::nullopt;
  }
...
bool ActionMap::SetDescription(std::string_view name,
