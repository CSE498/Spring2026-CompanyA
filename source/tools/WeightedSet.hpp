 ...
 * Credit notes: This files makes use parts of the Array implementation of a
 * Binary Tree described at:
 ...
 * https://stackoverflow.com/questions/1761626/weighted-random-numbers (top
 * answer)
 **/
...
template <typename T> class WeightedSet {
private:
  static constexpr double WEIGHTED_SET_TOL = 1e-12;

  // Storage should be relatively small assuming T will be a string or int
  // (probable for ids)
  ...
  void FixTinyNum(double &weight) {
    // Tiny weights below tolerance that are not 0 can cause strange sample
    // outputs
...
public:
  ...
  void Insert(const T &id, double weight) {
    if (weight < 0.0) {
      throw std::invalid_argument(
          "cse498::WeightedSet::Insert(): weight must be non-negative");
    }
    if (item_idx.contains(id)) {
      throw std::invalid_argument(
          "cse498::WeightedSet::Insert(): duplicate item");
    }
  ...
  void Update(const T &id, double weight) {
    if (weight < 0.0) {
      throw std::invalid_argument(
          "cse498::WeightedSet::Update(): weight must be non-negative");
    }

    if (!item_idx.contains(id)) {
      throw std::invalid_argument(
          "cse498::WeightedSet::Update(): item to update does not exist");
    }
  ...
  T Sample(double num) const {
    if (sum_tree.empty() || sum_tree[0] <= WEIGHTED_SET_TOL) {
      throw std::runtime_error("cse498::WeightedSet::Sample(): Cannot sample "
                               "from an empty WeightedSet");
    }
    if (num < 0 || num > sum_tree[0] + WEIGHTED_SET_TOL) {
      throw std::invalid_argument(
          "cse498::WeightedSet::Sample(): Sample number invalid");
    }
    ...
    while (true) {
      assert(num >= outer_lo - WEIGHTED_SET_TOL &&
             num <= outer_up + WEIGHTED_SET_TOL);
      ...
      double inner_up =
          inner_lo + weights[idx]; // Upper value in node's interval

      // Case 1: number in the left subtree interval
      if (num <= inner_lo + WEIGHTED_SET_TOL && left_idx < set_size &&
      ...
      if (right_idx >=
          set_size) { // for safety - this shouldnt run (Case 1 should have)
  ...
  double GetItemSum(const T &item) const {
    return sum_tree.at(item_idx.at(item));
  }
  double GetWeight(const T &item) const {
    return weights.at(item_idx.at(item));
  }
};
