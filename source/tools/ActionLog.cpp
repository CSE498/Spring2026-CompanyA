...
ActionLog::ActionLog() : NextSequenceNumber(0), CurrentTime(0.0) {}

void ActionLog::LogAction(int entityId, const std::string &actionType, double x,
...
const std::vector<Action> &ActionLog::GetActions() const { return Actions; }

std::vector<Action> ActionLog::GetActionRange(double startTime,
...
bool ExportToCsv(const ActionLog &log, const std::string &filePath) {
  ...
  for (const auto &action : log.GetActions()) {
    file << action.SequenceNumber << "," << action.EntityId << ","
         << action.ActionType << "," << action.Timestamp << "," << action.X
         << "," << action.Y << "," << action.NewX << "," << action.NewY << "\n";
  }
  return true;
}
...
std::string Serialize(const ActionLog &log) {
  ...
  for (const auto &action : log.GetActions()) {
    oss << action.SequenceNumber << " " << action.EntityId << " "
        << action.Timestamp << " " << action.X << " " << action.Y << " "
        << action.NewX << " " << action.NewY << " " << action.ActionType
        << "\n";
  }
  return oss.str();
}
...
void Deserialize(ActionLog &log, const std::string &data) {
  ...
  for (int i = 0; i < count; ++i) {
    Action action;
    if (iss >> action.SequenceNumber >> action.EntityId >> action.Timestamp >>
        action.X >> action.Y >> action.NewX >> action.NewY >>
        action.ActionType) {
      log.UpdateTime(action.Timestamp);
      log.LogAction(action.EntityId, action.ActionType, action.X, action.Y,
