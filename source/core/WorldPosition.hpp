...
private:
  ...
  /// chops off endings of doubles and converts to ints to make more
  /// consistent/comparable doubles
...
public:
  ...
  WorldPosition operator+(const PathVector &v) const {
    return {x + v.X(), y + v.Y()};
  }
  WorldPosition operator-(const PathVector &v) const {
    return {x - v.X(), y - v.Y()};
  }
  PathVector operator-(const WorldPosition &pos) const {
  ...
  [[nodiscard]] bool IsValid() const {
    return (
        x >= 0.0 &&
        y >= 0.0); // exists because above condition for CellX() CellY() exists.
  }
  ...
  static std::int64_t Quantize(double val) {
    return static_cast<std::int64_t>(std::llround(val / eps));
  }
  /**
   * uses CellX() and CellY() for positioning since this is used for determining
   * locations of world positions in the map based on GIVEN code. TODO: Change
   * CellX() CellY() to round() since this will feel more natural
   * TODO:                                 for this application of traversing
   * the world.
...
template <> struct std::hash<cse498::WorldPosition> {
  std::size_t operator()(const cse498::WorldPosition &pos) const noexcept {
    ...
    result ^= std::hash<std::int64_t>{}(qy) + 0x9e3779b9 + (result << 6) +
              (result >> 2);
    return result;
  }
};
