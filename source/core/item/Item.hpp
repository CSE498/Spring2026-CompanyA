...
public:
    Item(const size_t id, std::string name, std::string image_path, const int gold, const WorldBase& world) :
        m_id(id), m_name(std::move(name)), m_image_path(std::move(image_path)), m_gold(gold), m_world(world) {}
