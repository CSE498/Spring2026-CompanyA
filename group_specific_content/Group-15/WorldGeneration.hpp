...
protected:
    ...
    std::vector<LegacyLinkedRooms> mConnectedRooms; // The x-y coord pairs of two rooms used for connecting the room
                                                    // pair
...
public:
    ...
    LegacyWorldGeneration() :
        mBSP(), // For now, the constructor for BSP_tree room creation is going to generate rooms immediately when
                // initialized, will reformat as level specifications become more detailed
        mRoomHolder(), mGrid(mBSP.GetHeight(), std::string(mBSP.GetWidth(), '#'))
    ...
    /// @brief
    /// @return
    [[nodiscard]] std::vector<std::string> GetDungeon() const { return mGrid; }

    /// @brief
    /// @return
    [[nodiscard]] LegacyBSP GetBSP() const { return mBSP; }

    /// @brief
    /// @return
    [[nodiscard]] LegacyRoomHolder GetRoomholder() const { return mRoomHolder; }

    /// @brief
    /// @return
    [[nodiscard]] std::vector<LegacyLinkedRooms> GetConnectedRooms() const { return mConnectedRooms; }
};
} // namespace cse498
