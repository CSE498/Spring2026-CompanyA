...
public:
    /// @brief Constructor call creates the BSP Tree from the get-go, meaning that BSP_Tree and its leaf nodes are
    /// already populated
    ...
    int insert_split(int iter) {
        LegacyBSPNode root_node =
                LegacyBSPNode{-1, -1, 0, 0, mWidth, mHeight, std::string("Container")}; // creation of root node
    ...
    /// @brief
    /// @param iter
    void SetIterations(size_t iter) { mIterations = iter; }
    ...
    [[nodiscard]] size_t GetIterations() const { return mIterations; }

    // Returns the BSP_Tree's leaf nodes
    [[nodiscard]] std::vector<LegacyBSPNode> GetLeafNodes() const { return mLeafNodes; }

    // Returns the entirety of the created BSP_Tree
    [[nodiscard]] std::vector<LegacyBSPNode> GetBSPTree() const { return mBSP_Tree; }
    ...
    [[nodiscard]] std::optional<std::tuple<LegacyBSPNode, LegacyBSPNode>> random_split(LegacyBSPNode& node, int& iter) {
        ...
        /// Horizontal split
            ...
            right_split = {
                    ...
                    -1, // left right child
                    node.x,
                    node.y + left_split.height, // x-y coordinate
                    node.width, // width
