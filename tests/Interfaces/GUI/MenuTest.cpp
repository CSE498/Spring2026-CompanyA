...
TEST_CASE("Callback execution", "[Menu]") {
  ...
  menu.add_option("Increment", [&counter]() { counter++; });

  menu.add_option("Double", [&counter]() { counter *= 2; });
...
TEST_CASE("Error handling - empty label", "[Menu]") {
  ...
  REQUIRE_THROWS_AS(menu.add_option("", []() {}), std::invalid_argument);
}
...
TEST_CASE("Error handling - null callback", "[Menu]") {
  ...
  REQUIRE_THROWS_AS(menu.add_option("Test", nullptr), std::invalid_argument);
}
...
TEST_CASE("Error handling - invalid index selection", "[Menu]") {
  ...
  REQUIRE_THROWS_AS(menu.select_option(5), std::out_of_range);
}
...
TEST_CASE("Error handling - get invalid label index", "[Menu]") {
  ...
  REQUIRE_THROWS_AS(menu.get_option_label(10), std::out_of_range);
}
...
TEST_CASE("Error handling - activate with no selection", "[Menu]") {
  ...
  REQUIRE_THROWS_AS(menu.activate_selected(), std::runtime_error);
}
