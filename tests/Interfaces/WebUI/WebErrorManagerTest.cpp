...
static std::string GetLastAlert()
{
    return GetWindow()["__test_lastAlert"].as<std::string>();
}

static int GetAlertCount()
{
    return GetWindow()["__test_alertCount"].as<int>();
}
...
// ========================================================
TEST_CASE("Info does not throw", "[WebErrorManager][info]")
{
    REQUIRE_NOTHROW(
        WebErrorManager::Info("info test")
    );
}
...
{
    REQUIRE_NOTHROW(
        WebErrorManager::Warning("warning test")
    );
}
...
{
    REQUIRE_NOTHROW(
        WebErrorManager::Error("error test")
    );
}
...
TEST_CASE("Fatal throws when throwOnFatal is enabled", "[WebErrorManager][fatal]")
{
    ...
    REQUIRE_THROWS_AS(
        WebErrorManager::Fatal("fatal throw test", opts),
        std::runtime_error
    );
}
...
TEST_CASE("Fatal thrown exception contains message", "[WebErrorManager][fatal]")
{
    ...
    catch (const std::runtime_error& ex)
    {
        ...
        CHECK(msg.find("fatal message body") != std::string::npos,
              "runtime_error should include original message");
    }
}
...
TEST_CASE("Error with showAlert triggers alert", "[WebErrorManager][alert]")
{
    ...
    CHECK(GetLastAlert().find("alertable error") != std::string::npos,
          "alert text should contain original message");
}
...
TEST_CASE("Warning with showAlert triggers alert", "[WebErrorManager][alert]")
{
    ...
    CHECK(GetLastAlert().find("warning alert text") != std::string::npos,
          "alert text should contain warning message");
}
...
TEST_CASE("Repeated Error calls remain stable", "[WebErrorManager][repeat]")
{
    ...
    {
        REQUIRE_NOTHROW(
            WebErrorManager::Error("repeat error " + std::to_string(i))
        );
    }
}
...
TEST_CASE("Repeated Warning calls remain stable", "[WebErrorManager][repeat]")
{
    ...
    {
        REQUIRE_NOTHROW(
            WebErrorManager::Warning("repeat warning " + std::to_string(i))
        );
    }
}
...
TEST_CASE("Info with showAlert triggers alert", "[WebErrorManager][info][alert]")
{
    ...
    CHECK(GetLastAlert().find("info alert") != std::string::npos,
          "alert text should contain info message");
}

// ========================================================
// Test 12: Fatal with alert and throw both triggers alert first
// ========================================================
TEST_CASE("Fatal with showAlert and throwOnFatal alerts and throws",
          "[WebErrorManager][fatal][alert]")
{
    captureAlertInstallShim();
    InstallAlertCapture();
    ResetAlertCapture();
    ...
    {
        std::string msg = ex.what();
        CHECK(msg.find("fatal alert throw") != std::string::npos,
              "thrown exception should contain fatal message");
    }

    CHECK(GetAlertCount() == 1, "alert should be called before throw");
    CHECK(GetLastAlert().find("fatal alert throw") != std::string::npos,
          "alert should contain fatal message");
}
...
TEST_CASE("Empty message is handled safely", "[WebErrorManager][edge]")
{
    ...
    REQUIRE_NOTHROW(
        WebErrorManager::Warning("")
    );

    REQUIRE_NOTHROW(
        WebErrorManager::Info("")
    );
}
...
TEST_CASE("Browser console exists", "[WebErrorManager][emscripten]")
{
    ...
    CHECK((!console.isNull() && !console.isUndefined()),
          "global console object should exist");
}
