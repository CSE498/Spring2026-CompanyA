 ...
 *      2. Go to cd emsdk directory and run `emsdk install latest` and `emsdk
 * activate latest`.
 *      3. Run emsdk_env.bat to set up environment variables.
 *      4. cd "..\CSE 498\CompanyA\Spring2026-CompanyA\tests\Interfaces\WebUI"
 *      5. Compile with:`em++ WebTextboxTest.cpp
 * ../../../source/Interfaces/WebUI/WebTextbox/WebTextbox.cpp
 * ../../../source/Interfaces/WebUI/WebLayout/WebLayout.cpp -I../../../
 * -I../../../third-party/Catch/single_include --bind -std=c++23 -s WASM=1 -s
 * ASSERTIONS=1 -o test.html`
 *      6. Run python -m http.server 8000 for local hosting
 *      7. Open test.html in a browser to run the tests and view results in the
 * console (http://localhost:8000/test.html)
 *
 * Additional Note:
...
// static double GetBoundingClientRectNumber(val element, const std::string&
// key)
// {
...
// Make a unique root id for layouts in tests so tests dont interfere with each
// other in the DOM
...
// Already had a CHECK macro for internal use, map it to Catch2 REQUIRE with
// message support for better test output.
...
// ========================================================
TEST_CASE("MountToLayout makes element appear in DOM", "[webtextbox][dom]") {
...
// ========================================================
TEST_CASE("SetFontSize changes computed font-size", "[webtextbox][style]") {
  WebLayout root(UniqueRootId("layout"));
  root.SetLayoutType(LayoutType::Vertical);
  root.Apply();
...
// ========================================================
TEST_CASE("SetAlignment changes computed text-align", "[webtextbox][style]") {
  WebLayout root(UniqueRootId("layout"));
  root.SetLayoutType(LayoutType::Vertical);
  root.Apply();
...
// ========================================================
TEST_CASE("SetMaxWidth changes computed max-width", "[webtextbox][style]") {
  WebLayout root(UniqueRootId("layout"));
  root.SetLayoutType(LayoutType::Vertical);
  root.Apply();
...
// ========================================================
TEST_CASE("SetWrap toggles computed white-space", "[webtextbox][style]") {
  WebLayout root(UniqueRootId("layout"));
  root.SetLayoutType(LayoutType::Vertical);
  root.Apply();
...
// ========================================================
TEST_CASE("Bounding box is non-zero after mount", "[webtextbox][bbox]") {
  WebLayout root(UniqueRootId("layout"));
  root.SetLayoutType(LayoutType::Vertical);
  root.Apply();
