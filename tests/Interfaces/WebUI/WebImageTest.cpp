...
void test_constructor() {
    ...
    REQUIRE_MSG(img.GetSource() == "https://example.com/cat.png", "source mismatch");
...
void test_error_callback_with_placeholder() {
    ...
    img.SetOnErrorCallback([&img, &callback_saw_error]() { callback_saw_error = img.HasError(); });
...
void test_registry_load_event_after_move_constructor() {
    ...
    REQUIRE_MSG(WebImage_registryContains(registry_id) == 1, "original registry id should be registered before move");
    REQUIRE_MSG(original.IsLoaded() == false, "original should start unloaded");

    WebImage moved(std::move(original));

    CHECK_MSG(WebImageTestAccessor::RegistryId(original) == -1, "moved-from image should release its registry id");
    ...
    CHECK_MSG(moved.IsLoaded() == true, "load event should update the moved-to image");
    CHECK_MSG(moved.HasError() == false, "moved-to image should not report an error after load");
    CHECK_MSG(original.IsLoaded() == false, "moved-from image should not receive the load event");
    CHECK_MSG(original.HasError() == false, "moved-from image should remain error-free");
    PASS();
}
...
void test_registry_error_event_after_move_assignment() {
    ...
    CHECK_MSG(WebImageTestAccessor::RegistryId(src) == -1, "moved-from source should release its registry id");
    CHECK_MSG(WebImageTestAccessor::RegistryId(dest) == src_registry_id,
              "destination should adopt the source registry id");
    CHECK_MSG(WebImage_registryContains(dest_registry_id) == 0, "destination's old registry id should be unregistered");

    WebImage_handleError(src_registry_id);

    CHECK_MSG(dest.HasError() == true, "error event should update the move-assigned destination");
    CHECK_MSG(dest.IsLoaded() == false, "destination should remain unloaded after error");
    CHECK_MSG(src.HasError() == false, "moved-from source should not receive the error event");
    CHECK_MSG(src.IsLoaded() == false, "moved-from source should remain unloaded");
    PASS();
}
...
void test_registry_unregistered_after_moved_image_destruction() {
    ...
    REQUIRE_MSG(transferred_registry_id >= 0, "test should capture a valid transferred registry id");
...
void test_registry_unregistered_after_plain_destruction() {
    ...
    CHECK_MSG(WebImage_registryContains(registry_id) == 0, "destroying the image should unregister its registry id");
    PASS();
}
...
TEST_CASE_METHOD(SharedWebContext, "SetSource updates DOM src and clears placeholder styling", "[WebImage][dom]") {
    ...
    CHECK_MSG(GetCSSProperty(el, "background-color").empty(), "SetSource should clear placeholder background styling");
}

TEST_CASE_METHOD(SharedWebContext, "SetSize zero leaves DOM width and height unset", "[WebImage][dom][style]") {
    root.SetLayoutType(LayoutType::Vertical);
    root.Apply();
...
TEST_CASE_METHOD(SharedWebContext, "BlankRect mode applies placeholder rectangle in DOM", "[WebImage][dom][error]") {
    root.SetLayoutType(LayoutType::Vertical);
    root.Apply();
...
TEST_CASE_METHOD(SharedWebContext, "SetErrorMode changes DOM placeholder behavior", "[WebImage][dom][error]") {
    root.SetLayoutType(LayoutType::Vertical);
    root.Apply();
    ...
    CHECK_MSG(GetCSSProperty(el, "display").empty(), "NoOp mode should not force inline-block placeholder display");
...
TEST_CASE_METHOD(SharedWebContext, "SetPlaceholderColor changes placeholder color in DOM", "[WebImage][dom][error]") {
    root.SetLayoutType(LayoutType::Vertical);
    root.Apply();
...
TEST_CASE_METHOD(SharedWebContext, "SyncFromModel restores DOM attributes and styles", "[WebImage][dom]") {
    root.SetLayoutType(LayoutType::Vertical);
    root.Apply();
    ...
    CHECK_MSG(GetCSSProperty(el, "opacity").rfind("0.8", 0) == 0, "SyncFromModel should restore the DOM opacity");
}
// ========================================================
// Test 40: SetAltText updates mounted DOM alt attribute
// ========================================================
TEST_CASE_METHOD(SharedWebContext, "SetAltText updates DOM alt attribute", "[WebImage][dom]") {
    root.SetLayoutType(LayoutType::Vertical);
    root.Apply();
    ...
    img.SetAltText("new alt");
    CHECK_MSG(GetAttributeStr(el, "alt") == "new alt", "SetAltText should update the DOM alt attribute");
}

// ========================================================
// Test 41: SetSize applies width height and object-fit fill in DOM
// ========================================================
TEST_CASE_METHOD(SharedWebContext, "SetSize applies DOM width height and fill object-fit", "[WebImage][dom][style]") {
    root.SetLayoutType(LayoutType::Vertical);
    root.Apply();
    ...
    CHECK_MSG(GetCSSProperty(el, "width") == "210px", "SetSize should apply width to the DOM");
    CHECK_MSG(GetCSSProperty(el, "height") == "140px", "SetSize should apply height to the DOM");
    CHECK_MSG(GetCSSProperty(el, "object-fit") == "fill", "SetSize should apply object-fit fill");
}

// ========================================================
// Test 42: Resize with aspect ratio uses contain object-fit
// ========================================================
TEST_CASE_METHOD(SharedWebContext, "Resize with maintain aspect ratio uses contain", "[WebImage][dom][style]") {
    root.SetLayoutType(LayoutType::Vertical);
    root.Apply();
    ...
    CHECK_MSG(GetCSSProperty(el, "width") == "320px", "Resize should apply width to the DOM");
    CHECK_MSG(GetCSSProperty(el, "height") == "180px", "Resize should apply height to the DOM");
    CHECK_MSG(GetCSSProperty(el, "object-fit") == "contain", "Resize(true) should apply object-fit contain");
}

// ========================================================
// Test 43: Resize without aspect ratio uses fill object-fit
// ========================================================
TEST_CASE_METHOD(SharedWebContext, "Resize without maintain aspect ratio uses fill", "[WebImage][dom][style]") {
    root.SetLayoutType(LayoutType::Vertical);
    root.Apply();
    ...
    CHECK_MSG(GetCSSProperty(el, "width") == "111px", "Resize should apply width to the DOM");
    CHECK_MSG(GetCSSProperty(el, "height") == "77px", "Resize should apply height to the DOM");
    CHECK_MSG(GetCSSProperty(el, "object-fit") == "fill", "Resize(false) should apply object-fit fill");
}

// ========================================================
// Test 44: SetOpacity updates DOM style immediately
// ========================================================
TEST_CASE_METHOD(SharedWebContext, "SetOpacity updates DOM opacity immediately", "[WebImage][dom][style]") {
    root.SetLayoutType(LayoutType::Vertical);
    root.Apply();
...
// ========================================================
TEST_CASE_METHOD(SharedWebContext, "Show and Hide update DOM display immediately", "[WebImage][dom][visibility]") {
    root.SetLayoutType(LayoutType::Vertical);
    root.Apply();
    ...
    img.Hide();
    CHECK_MSG(GetCSSProperty(el, "display") == "none", "Hide should immediately set display none");

    img.Show();
    CHECK_MSG(GetCSSProperty(el, "display").empty(), "Show should clear the inline display override");
}
...
TEST_CASE_METHOD(SharedWebContext, "BlankRect placeholder uses default size when image size is unset",
    ...
    CHECK_MSG(GetCSSProperty(el, "width") == "100px", "placeholder width should fall back to the default size");
    CHECK_MSG(GetCSSProperty(el, "height") == "100px", "placeholder height should fall back to the default size");
...
TEST_CASE_METHOD(SharedWebContext, "SetSource after error clears error state and restores src",
    ...
    REQUIRE_MSG(GetCSSProperty(el, "display") == "inline-block", "placeholder should force inline-block display");
    ...
    CHECK_MSG(GetAttributeStr(el, "src") == kValidImageSrc, "SetSource should restore the DOM src");
    CHECK_MSG(GetCSSProperty(el, "background-color").empty(), "SetSource should clear placeholder background styling");
}
...
// ========================================================
TEST_CASE_METHOD(SharedWebContext, "HandleLoad updates state after placeholder error", "[WebImage][dom][error]") {
    root.SetLayoutType(LayoutType::Vertical);
    root.Apply();
