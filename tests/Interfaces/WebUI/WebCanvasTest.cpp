...
// Notes:
// - Do NOT define CATCH_CONFIG_MAIN here if another test file already defines
// it.
...
// This avoids pulling in the real WebLayout implementation (which may depend on
// emscripten).
...
TEST_CASE("WebCanvas Id() reflects constructor argument, and mount/unmount do "
          "not change Id()",
          "[web][canvas][dom]") {
...
TEST_CASE("WebCanvas AddElement(nullptr) is safe and does not affect ownership "
          "counts",
          "[web][canvas][raii]") {
...
TEST_CASE("WebCanvas owns added elements and releases them on clear/destroy "
          "(stronger assertions)",
          "[web][canvas][raii]") {
  LifetimeElement::reset();
  REQUIRE(LifetimeElement::alive == 0);
...
TEST_CASE("WebCanvas RenderFrame draws visible elements, skips invisible, and "
          "does not clear elements",
          "[web][canvas]") {
  WebCanvas canvas;
...
TEST_CASE("WebCanvas RenderFrame sorts by ZIndex (stable), including equal "
...
TEST_CASE(
    "WebCanvas ZIndex changes after insertion affect subsequent render order",
    "[web][canvas][zindex]") {
  WebCanvas canvas;
...
TEST_CASE("WebCanvas immediate-mode primitives are safe to call (expanded)",
          "[web][canvas][primitives]") {
  WebCanvas canvas("canvas-test-primitives");

  // These are no-ops in native builds; in Emscripten builds they forward to
  // Canvas2D. Either way, they should be safe to invoke with a wide range of
  // inputs.
  ...
  REQUIRE_NOTHROW(
      canvas.DrawCircle(50, 50, 25, "#00ff00", 0.0f, "#001100")); // no stroke
  REQUIRE_NOTHROW(
      canvas.DrawCircle(50, 50, 25, "#00ff00", 3.0f, "")); // no fill
  ...
  REQUIRE_NOTHROW(canvas.DrawPolygon(
      empty, "#fff", 1.0f, "#000")); // 0 points -> should early-return

  std::vector<WebCanvas::Vec2> one{{0, 0}};
  REQUIRE_NOTHROW(canvas.DrawPolygon(one, "#fff", 1.0f,
                                     "#000")); // 1 point -> should early-return

  std::vector<WebCanvas::Vec2> two{{0, 0}, {10, 0}};
  REQUIRE_NOTHROW(canvas.DrawPolygon(
      two, "#fff", 1.0f,
      "#000")); // 2 points -> allowed, implementation-defined
}