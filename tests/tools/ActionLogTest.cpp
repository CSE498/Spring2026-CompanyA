...
static void FillLog(cse498::ActionLog& log, int entityId, int count) {
    for (int i = 0; i < count; ++i) {
        ...
        log.LogAction(entityId, "move", cse498::WorldPosition{static_cast<double>(i), 0.0},
...
TEST_CASE("ActionLog::GetEntityActions filters by entity ID", "[actionlog][getentity]") {
    ...
    SECTION("Unknown entity returns empty list") { CHECK(log.GetEntityActions(99).empty()); }
}
...
TEST_CASE("ActionLog::GetActionRange filters by time range", "[actionlog][range]") {
    ...
    SECTION("Range with no matches returns empty") { CHECK(log.GetActionRange(5.0, 10.0).empty()); }
}
...
TEST_CASE("ActionLog::IsEntityStuck detects stationary entities", "[actionlog][stuck]") {
    ...
    SECTION("Entity that moves is not stuck") {
        for (int i = 0; i < 5; ++i) {
            log.UpdateTime(static_cast<double>(i));
            log.LogAction(1, "move", {static_cast<double>(i), 0.0}, {static_cast<double>(i + 1), 0.0});
        }
    ...
    SECTION("Unknown entity returns false") { CHECK_FALSE(log.IsEntityStuck(99, 3)); }
}
...
TEST_CASE("AgentActionLog::GetStuckAgentRatio reports correct proportion", "[agentlog][stuck]") {
    ...
    for (int i = 0; i < 5; ++i) {
        log.UpdateTime(static_cast<double>(i));
        log.LogAction(2, "move", {static_cast<double>(i), 0.0}, {static_cast<double>(i + 1), 0.0});
    }
...
TEST_CASE("UserActionLog::GetLastAction returns the most recent entry", "[userlog][last]") {
    ...
    SECTION("Empty log returns nullopt") { CHECK_FALSE(log.GetLastAction().has_value()); }
...
TEST_CASE("UserActionLog::GetMostFrequentActionType identifies dominant action", "[userlog][freq]") {
    ...
    SECTION("Empty log returns nullopt") { CHECK_FALSE(log.GetMostFrequentActionType().has_value()); }
...
TEST_CASE("ActionLog handles a large number of actions correctly", "[actionlog][stress]") {
    ...
    SECTION("All actions are recorded") { CHECK(log.GetActionCount() == ACTION_COUNT); }
...
TEST_CASE("ActionLog::GetMostActiveEntity returns entity with most actions", "[actionlog][mostactive]") {
    ...
    SECTION("Empty log returns nullopt") { CHECK_FALSE(log.GetMostActiveEntity().has_value()); }
