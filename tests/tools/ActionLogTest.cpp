...
TEST_CASE("ActionLog::GetEntityActions filters by entity ID",
          "[actionlog][getentity]") {
  cse498::ActionLog log;
...
TEST_CASE("ActionLog::GetActionRange filters by time range",
          "[actionlog][range]") {
  cse498::ActionLog log;
...
TEST_CASE("ActionLog::Clear resets action list and sequence counter",
          "[actionlog][clear]") {
  cse498::ActionLog log;
...
TEST_CASE("ActionLog::IsEntityStuck detects stationary entities",
          "[actionlog][stuck]") {
  cse498::ActionLog log;
  ...
  SECTION("Entity that moves is not stuck") {
    for (int i = 0; i < 5; ++i) {
      log.UpdateTime(static_cast<double>(i));
      log.LogAction(1, "move", {static_cast<double>(i), 0.0},
...
TEST_CASE("ActionLog serialize and deserialize round-trips correctly",
          "[actionlog][serialize]") {
...
TEST_CASE("AgentActionLog::GetStuckAgentRatio reports correct proportion",
          "[agentlog][stuck]") {
  ...
  for (int i = 0; i < 5; ++i) {
    log.UpdateTime(static_cast<double>(i));
    log.LogAction(2, "move", {static_cast<double>(i), 0.0},
                  {static_cast<double>(i + 1), 0.0});
  }
...
TEST_CASE("UserActionLog::GetLastAction returns the most recent entry",
          "[userlog][last]") {
  ...
  SECTION("Returns the last logged action") {
    log.UpdateTime(1.0);
    log.LogAction(0, "jump", {0, 0}, {0, 1});
    log.UpdateTime(2.0);
    log.LogAction(0, "run", {0, 1}, {5, 1});
...
TEST_CASE("UserActionLog::GetMostFrequentActionType identifies dominant action",
          "[userlog][freq]") {
  cse498::UserActionLog log;
  ...
  SECTION("Returns the most common action type") {
    log.UpdateTime(0.0);
    log.LogAction(0, "move", {0, 0}, {1, 0});
    log.UpdateTime(1.0);
    log.LogAction(0, "move", {1, 0}, {2, 0});
    log.UpdateTime(2.0);
    log.LogAction(0, "jump", {2, 0}, {2, 1});
    log.UpdateTime(3.0);
    log.LogAction(0, "move", {2, 1}, {3, 1});
...
TEST_CASE("ActionLog handles a large number of actions correctly",
          "[actionlog][stress]") {
  ...
  SECTION("GetActionRange across full range returns everything") {
    auto result =
        log.GetActionRange(0.0, static_cast<double>(ACTION_COUNT - 1));
...
TEST_CASE("ActionLog::GetMostActiveEntity returns entity with most actions",
          "[actionlog][mostactive]") {
  cse498::ActionLog log;
...
TEST_CASE("ActionLog::GetActionCountInRange returns correct count",
          "[actionlog][countrange]") {
  cse498::ActionLog log;
