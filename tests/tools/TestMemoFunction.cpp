...
TEST_CASE("MemoFunction: Cache limit", "[MemoFunction]") {
    ...
    mf9.SetLimit(3); // SetLimit now triggers eviction, which now evicts more than one
    REQUIRE(mf9.CacheSize() == 3);
    REQUIRE(!mf9.IsCached(1));
    ...
    REQUIRE(mf9.CacheSize() == 10000); // int max is 2147483647 on most systems, no eviction as
                                       // size is well within limit
}
