...
TEST_CASE("Timer Constructor", "[Timer]")
{
    ...
    // Test that adding true in the constructor functions properly and starts the Timer running.
...
{
    // Create a Timer stopped at creation and double check that the state is as intended.
    cse498::Timer Timer("Test1", false);
    REQUIRE(!Timer.isRunning());
    REQUIRE(Timer.elapsed() == 0.0);

    // Call start on the Timer and make sure that it is running and that time is elapsing.
    Timer.start();
    REQUIRE(Timer.isRunning());
    REQUIRE(Timer.elapsed() > 0);
...
TEST_CASE("Timer Stop Method", "[Timer]")
{
    ...
    // Make sure that calling stop on a paused Timer doesn't change the state or increase the elapsed time.
    Timer.stop();
...
TEST_CASE("Timer Elapsed Method", "[Timer]")
{
    ...
    // Advance time while the Timer is paused and make sure it doesn't increase the elapsed time.
    Timer.advanceTime(1.0);
    REQUIRE(Timer.elapsed() == Approx(2.0).margin(0.01));

    // Start the Timer back up and advance time by some less conformative intervals.
    Timer.start();
...
TEST_CASE("Timer Reset Method", "[Timer]")
{
    ...
    // Test that reset still functions properly when called on a running Timer as opposed to a stopped one.
...
TEST_CASE("Timer Restart Method", "[Timer]")
{
    ...
    // Test that restart still functions properly when called on a running Timer as opposed to a stopped one.
    cse498::Timer Timer2("Test2");
    REQUIRE(Timer2.isRunning());
...
{
    // Start a Timer and make sure that the first lap time is updating properly alongside the Timer.
    cse498::Timer Timer("Test1");
    REQUIRE(Timer.isRunning());
    Timer.stop();
    ...
    // make sure that another lap was added to the Timer, and that the first one properly recorded time.
    Timer.start();
