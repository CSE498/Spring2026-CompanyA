...
TEST_CASE("WorldPath: SelfIntersects false for simple polyline",
          "[WorldPath]") {
  WorldPath p;
  p.AddPoint(WP(0, 0));
  p.AddPoint(WP(1, 0));
...
TEST_CASE("WorldPath: SelfIntersects true for crossing segments",
          "[WorldPath]") {
...
TEST_CASE("WorldPath: FurthestPointPair returns nullopt for <2 points",
          "[WorldPath]") {
  WorldPath p;
  REQUIRE_FALSE(p.FurthestPointPair().has_value());
...
TEST_CASE("WorldPath: FurthestPointPair returns correct indices",
          "[WorldPath]") {
  // Points: 0:(0,0), 1:(0,3), 2:(4,0)  -> farthest pair is (1,2) dist=5 OR
  // (0,2) dist=4, (0,1) dist=3
  WorldPath p;
  p.AddPoint(WP(0, 0));
  p.AddPoint(WP(0, 3));
  p.AddPoint(WP(4, 0));
  ...
  // Check that the chosen pair is actually the farthest by verifying distance
  // is 5.
...
TEST_CASE("WorldPath: Extend appends points and skips consecutive duplicates",
          "[WorldPath]") {
  WorldPath a;
  a.AddPoint(WP(0, 0));
  a.AddPoint(WP(1, 0));
...
TEST_CASE("WorldPath: operator== compares paths by point sequence",
          "[WorldPath]") {
  WorldPath a;
  a.AddPoint(WP(0, 0));
  a.AddPoint(WP(1, 1));
