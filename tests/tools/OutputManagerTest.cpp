...
/** @brief Extracts the leading timestamp in milliseconds from a formatted log
 * line.
...
/*@brief Captures anything written to std::cout during the lifetime of this
 * object
 */
class CoutCapture {
public:
 ...
 */
TEST_CASE("OutputManager::GetMinLogLevel returns current min level",
          "[output][minlevel]") {
 ...
 */
TEST_CASE("OutputManager::ShouldLog respects minimum log level",
          "[output][shouldlog]") {
  cse498::OutputManager om;
 ...
 */
TEST_CASE("OutputManager::Log is filtered when below minimum level",
          "[output][log][filter]") {
  cse498::OutputManager om;

  CoutCapture cap;
  om.SetMinLogLevel(cse498::LogLevel::Warn);
  om.Log(cse498::LogLevel::Info, cse498::LogCategory::System,
...
TEST_CASE(
    "OutputManager::Log formats level/category/message (timestamps disabled)",
    "[output][log][format]") {
  cse498::OutputManager om;
  ...
  CoutCapture cap;
  om.Log(cse498::LogLevel::Info, cse498::LogCategory::Combat,
         "Goblin hit player");
 ...
 * Timer integration
 */
TEST_CASE("OutputManager timestamp toggling affects output",
...
TEST_CASE("OutputManager timestamps advance over time using the shared Timer",
          "[output][log][timer]") {
  cse498::OutputManager om;
...
TEST_CASE("OutputManager can reset the timestamp clock",
          "[output][log][reset-timestamp]") {
  cse498::OutputManager om;
  std::vector<std::string> lines;
...
TEST_CASE("OutputManager convenience logging uses default level and category",
          "[output][log][defaults]") {
  cse498::OutputManager om;
  std::vector<std::string> lines;
...
TEST_CASE("OutputManager supports runtime category strings",
          "[output][log][runtime-category]") {
  cse498::OutputManager om;
  std::vector<std::string> lines;
...
TEST_CASE("OutputManager sink behavior", "[output][sink]") {
  ...
  SECTION("Per-sink minimum level filters independently") {
    ...
    om.ClearSinks();
    om.AddSink([&](std::string_view s) { lines.emplace_back(s); },
               cse498::LogLevel::Info);
    om.AddSink([&](std::string_view s) { warnOnlyLines.emplace_back(s); },
               cse498::LogLevel::Warn);
 ...
 */
TEST_CASE("OutputManager name helpers return correct strings",
          "[output][names]") {
  CHECK(cse498::OutputManager::LevelName(cse498::LogLevel::Debug) == "DEBUG");
  CHECK(cse498::OutputManager::LevelName(cse498::LogLevel::Verbose) ==
        "VERBOSE");
  ...
  CHECK(cse498::OutputManager::CategoryName(cse498::LogCategory::System) ==
        "SYSTEM");
  CHECK(cse498::OutputManager::CategoryName(cse498::LogCategory::WorldGen) ==
        "WORLDGEN");
  CHECK(cse498::OutputManager::CategoryName(cse498::LogCategory::Performance) ==
        "PERF");
  CHECK(cse498::OutputManager::CategoryName(cse498::LogCategory::Network) ==
        "NET");
}

/* @brief Tests for file path validation and state helpers
 */
TEST_CASE("OutputManager file path validation throws on invalid inputs",
          "[output][file][errors]") {
  cse498::OutputManager om;
...
TEST_CASE("OutputManager state helpers report current settings",
          "[output][state]") {
  cse498::OutputManager om;
