...
TEST_CASE("Testing DataFileManager FormatData", "[core]") {
  ...
  CHECK(manager.FormatData(3, "Tile", "Temporary Data") ==
        "3\tTile\tTemporary Data");
  std::remove("DataFileManagerTest.csv");
}

/*
 * Test case for the Update function of the DataFileManager class,
 * ensuring that it successfully writes the current state of the world to the
 * specified file.
 */
...
/*
 * Test case (error handling) for the FormatData function of the DataFileManager
 * class, verifying that it throws a runtime error when an invalid type is
 * provided.
 */
TEST_CASE("Testing DataFileManager FormatData with invalid type", "[core]") {
  ...
  CHECK_THROWS_AS(manager.FormatData(3, "InvalidType", "Temporary Data"),
                  std::runtime_error);
  std::remove("DataFileManagerTest.csv");
}
...
/*
 * Test case (error handling) for the FormatData function of the DataFileManager
 * class, ensuring that it throws a runtime error when an unconvertible data
 * type is provided.
 */
TEST_CASE("Testing DataFileManager FormatData with unconvertible data type",
          "[core]") {
  std::unique_ptr world = std::make_unique<cse498::MazeWorld>();
  cse498::DataFileManager manager("DataFileManagerTest.csv", world.get());
...
/*
 * Test case (error handling) for the FormatData function of the DataFileManager
 * class, verifying that it throws a runtime error when an empty type string is
 * provided.
 */
TEST_CASE("Testing DataFileManager FormatData when type is empty", "[core]") {
  ...
  CHECK_THROWS_AS(manager.FormatData(3, "", "Temporary Data"),
                  std::runtime_error);
  std::remove("DataFileManagerTest.csv");
}

/*
 * Test case (error handling) for the Update function of the DataFileManager
 * class, ensuring that it returns false when it fails to open the specified
 * file for writing.
 */
TEST_CASE("Testing DataFileManager Update with invalid file", "[core]") {
  std::unique_ptr world = std::make_unique<cse498::MazeWorld>();
  cse498::DataFileManager manager("/invalid_path/DataFileManagerTest.csv",
                                  world.get());
...
TEST_CASE("Testing DataFileManager Update with multiple updates", "[core]") {
  ...
  CHECK(line_count >= 4); // At least 4 lines expected after two updates, since
                          // each update writes two lines
