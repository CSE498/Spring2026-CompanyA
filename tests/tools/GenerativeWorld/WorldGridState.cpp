...
//#include "../../../source/core/WorldGridState.hpp"

#include "../../../source/tools/WorldGridState.hpp"
...
void AddBasicCellTypes(cse498::WorldGridState &grid) {
...
cse498::WorldGridState MakeSmallGrid3x2() {
    cse498::WorldGridState grid(3, 2);
    AddBasicCellTypes(grid);
    return grid;
}


TEST_CASE("WorldGridState Constructor", "[core]") {
    SECTION("Default constructor yields empty grid, but has Unknown type") {
        cse498::WorldGridState grid;
    ...
    SECTION("Sized constructor sets width/height and correct cell count") {
        cse498::WorldGridState grid(4, 3);
...
TEST_CASE("Indexing and mutation via operator[] with (x,y) pair", "[core]") {
    ...
    cse498::WorldGridState grid = MakeSmallGrid3x2();
