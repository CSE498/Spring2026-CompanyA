...
TEST_CASE("Test Insert", "[core]") {
  ...
  SECTION("Bad Inserts") {
    ...
    REQUIRE_THROWS_AS(wSet.Insert("E", -3.5), std::invalid_argument);

    REQUIRE_THROWS_AS(wSet.Insert("A", 5.0), std::invalid_argument);
  }
}
...
TEST_CASE("Test Sums", "[core]") {
  SECTION("Small Tree") {
    cse498::WeightedSet<std::string> wSet;
    MakeSmallTree(wSet);
    CHECK(wSet.GetItemSum("A") ==
          Approx(17.0)); // A = weight (2.0) + left sum (10.0) + right sum (5.0)
    CHECK(wSet.GetItemSum("B") ==
          Approx(10.0)); // B = weight (3.0) + Left sum (7.0) + right sum (0.0)
    CHECK(wSet.GetItemSum("C") ==
          Approx(5.0)); // C is a leaf, sum = weight (5.0)
    CHECK(wSet.GetItemSum("D") ==
          Approx(7.0)); // D is a leaf, sum = weight (7.0)
  }
  ...
  SECTION("Decimal tree") {
    ...
    CHECK(wSet.GetItemSum('a') == Approx(2.000000000001 + 3.023344563390414 +
                                         7.003450023394053 + 4.999999999999999)
                                      .margin(1e-12));
    CHECK(wSet.GetItemSum('b') ==
          Approx(3.023344563390414 + 7.003450023394053).margin(1e-12));
  ...
  SECTION("Tiny Tree") {
    ...
    CHECK(wSet.GetItemSum(1) ==
          Approx(0.000000010 + 0.000000009).margin(1e-12));
...
TEST_CASE("Test Update", "[core]") {
  ...
  SECTION("Bad Updates") {
    ...
    REQUIRE_THROWS_AS(wSet.Update("A", -3.5), std::invalid_argument);

    REQUIRE_THROWS_AS(wSet.Update("E", 5.0), std::invalid_argument);
  }
}
...
TEST_CASE("Test Sample", "[core]") {
  ...
  SECTION("Larger Tree") {
    ...
    CHECK(wSet.Sample(1.000000000002) == 9); // item 4 gets skipped because w =
                                             // 0
  ...
  SECTION("Weight Zero In Small tree") {
    ...
    REQUIRE_THROWS_AS(wSet.Sample(-7.0), std::invalid_argument);

    REQUIRE_THROWS_AS(wSet.Sample(20.0), std::invalid_argument);
    ...
    // All 0
    wSet.Update("C", 0.0);
    REQUIRE_THROWS_AS(wSet.Sample(0.0), std::runtime_error);
    ...
    REQUIRE_THROWS_AS(wSet.Sample(7.0 + 2.0 + 3.0 + 5.0),
    ...
    CHECK(wSet.Sample(7.0) == "D");
    CHECK(wSet.Sample(7.0 + 3.0) == "B");
    REQUIRE_THROWS_AS(wSet.Sample(7.0 + 2.0 + 3.0), std::invalid_argument);
  }
