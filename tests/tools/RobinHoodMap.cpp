...
// coverage *used opus 4.6 as well!
...
TEST_CASE("Insert returns false and updates on duplicate key",
          "[tools][RobinHoodMap]") {
  cse498::RobinHoodMap<int, int> map;
...
TEST_CASE("Iterator pre-increment and post-increment",
          "[tools][RobinHoodMap]") {
  cse498::RobinHoodMap<int, int> map;
  map.Insert(1, 10);
  map.Insert(2, 20);
...
TEST_CASE("Insertions after Reserve stay within load factor",
          "[tools][RobinHoodMap]") {
  cse498::RobinHoodMap<int, int> map;
  map.Reserve(100);
...
TEST_CASE("Insert, erase half, verify remainder (1000 elements)",
          "[tools][RobinHoodMap]") {
  cse498::RobinHoodMap<int, int> map;
  const int N = 1000;
...
TEST_CASE("Elements with colliding hashes are all retrievable",
          "[tools][RobinHoodMap]") {
...
TEST_CASE("Heavy collision with tiny initial capacity",
          "[tools][RobinHoodMap]") {
...
TEST_CASE("Erase under heavy load preserves remaining elements",
          "[tools][RobinHoodMap]") {
...
TEST_CASE("Custom equality comparator (case-insensitive strings)",
          "[tools][RobinHoodMap]") {
  ...
  struct CIHash {
    size_t operator()(const std::string &s) const {
      ...
      for (char c : s) {
        h = h * 31 +
            static_cast<size_t>(static_cast<unsigned char>(std::tolower(c)));
      }
      return h;
    }
  };
...
TEST_CASE("Iteration count matches Size after rehashes",
          "[tools][RobinHoodMap]") {
  cse498::RobinHoodMap<int, int> map(2);
  for (int i = 0; i < 500; ++i) {
    map.Insert(i, i);
