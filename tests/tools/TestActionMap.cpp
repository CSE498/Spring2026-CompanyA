...
TEST_CASE("Typed actions: arguments + return values", "[ActionMap]") {
    ...
    REQUIRE(map.AddFunction<int, int, int>("Add", std::function<int(int, int)>([](int a, int b) { return a + b; }),
                                           "Adds two ints"));
    ...
    int sink = 0;
    REQUIRE(map.AddFunction<void, int>("SetSink", std::function<void(int)>([&](int v) { sink = v; })));
