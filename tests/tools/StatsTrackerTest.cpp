...
TEST_CASE("Build Series Summary", "[StatsTracker]") {
    ...
    cse498::DataLog log;
    REQUIRE(tracker.BuildSeriesSummary("test_key", "Test Key", log) == std::nullopt);

    log.Add(4.5);
    log.Add(2.13);
    log.Add(43.07);

    cse498::StatSummary summary = tracker.BuildSeriesSummary("test_key", "Test Key", log).value();
...
TEST_CASE("Build Action Summary", "[StatsTracker]") {
    ...
    REQUIRE(tracker.BuildActionSummary("test_key", "Test Key", aLog) == std::nullopt);

    aLog.LogAction(1, "TestMove", cse498::WorldPosition(1, 2), cse498::WorldPosition(2, 1));
    aLog.LogAction(1, "TestMove2", cse498::WorldPosition(2, 1), cse498::WorldPosition(3, 3));
    aLog.LogAction(1, "TestMove3", cse498::WorldPosition(3, 3), cse498::WorldPosition(4, 3));

    cse498::ActionSummary summary = tracker.BuildActionSummary("test_key", "Test Key", aLog).value();
    REQUIRE(summary.key == "test_key");
    REQUIRE(summary.label == "Test Key");
