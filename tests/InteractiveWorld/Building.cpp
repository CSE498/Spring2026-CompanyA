...
#include "../../source/core/Building.hpp"
...
TEST_CASE("Test Building One Upgrade", "[core][InteractiveWorld][Building]")
{
	...
	CHECK(status2.error() == cse498::Building::UpgradeRejectionType::IncorrectItemType);
	CHECK(farm.GetCurrentLevel() == 0);
	...
	CHECK(status4.error() == cse498::Building::UpgradeRejectionType::AlreadyMaxLevel);
	CHECK(farm.GetCurrentLevel() == 1);
	CHECK(farm.IsMaxLevel() == true);
}
