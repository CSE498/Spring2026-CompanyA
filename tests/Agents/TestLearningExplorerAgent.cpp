...
TEST_CASE("LearningExplorerAgent uses BFS guidance toward nearest unvisited cell", "[LearningExplorerAgent]") {
    MazeWorld world;
    auto& agent = world.AddAgent<LearningExplorerAgent>("Explorer");
    agent.SetLocation(WorldPosition{6, 1});

    // First SelectAction marks the starting cell visited and computes a BFS-guided move.
    size_t action = agent.SelectAction(world.GetGrid());
    ...
    WorldPosition after = agent.GetLocation().AsWorldPosition();

    CHECK(result == 1);
    CHECK(world.GetGrid().IsValid(after));
    CHECK(world.GetGrid()[after] != world.GetGrid().GetCellTypeID("wall"));
    CHECK_FALSE(after == WorldPosition{6, 1});
}

TEST_CASE("LearningExplorerAgent does not immediately return to previous position when alternatives exist",
          "[LearningExplorerAgent]") {
    MazeWorld world;
    auto& agent = world.AddAgent<LearningExplorerAgent>("Explorer");
    agent.SetLocation(WorldPosition{6, 1});

    WorldPosition start = agent.GetLocation().AsWorldPosition();

    size_t first_action = agent.SelectAction(world.GetGrid());
    int first_result = world.DoAction(agent, first_action);
    agent.SetActionResult(first_result);

    REQUIRE(first_result == 1);

    WorldPosition second_pos = agent.GetLocation().AsWorldPosition();

    size_t second_action = agent.SelectAction(world.GetGrid());
    int second_result = world.DoAction(agent, second_action);
    agent.SetActionResult(second_result);

    REQUIRE(second_result == 1);

    WorldPosition third_pos = agent.GetLocation().AsWorldPosition();

    CHECK_FALSE(third_pos == start);
    CHECK_FALSE(third_pos == second_pos);
}

TEST_CASE("LearningExplorerAgent uses BFS guidance when neighboring cells are available", "[LearningExplorerAgent]") {
    MazeWorld world;
    auto& agent = world.AddAgent<LearningExplorerAgent>("Explorer");
...
TEST_CASE("LearningExplorerAgent avoids proximity to enemy agent", "[LearningExplorerAgent]") {
    ...
    int dx = static_cast<int>(new_pos.CellX()) - static_cast<int>(enemy_pos.CellX());
    int dy = static_cast<int>(new_pos.CellY()) - static_cast<int>(enemy_pos.CellY());
