...
public:
  ...
  void InvokePrintWorldState(const std::string &header) const {
    PrintWorldState(header);
  }
  ...
  bool InvokeIsAgentAt(WorldPosition pos,
                       size_t skip_agent_id = static_cast<size_t>(-1)) const {
    return IsAgentAt(pos, skip_agent_id);
  }

  std::optional<size_t> InvokeItemOnFloor(WorldPosition pos) const {
    return ItemOnFloor(pos);
  }
};
