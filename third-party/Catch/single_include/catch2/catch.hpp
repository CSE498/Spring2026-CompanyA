...
#    if defined( __CYGWIN__ ) || defined( __QNX__ ) || \
        defined( __EMSCRIPTEN__ ) || defined( __DJGPP__ )
...
#        define INTERNAL_CATCH_STRINGIZE_WITHOUT_PARENS( param ) \
            ( INTERNAL_CATCH_STRINGIZE(                          \
                  INTERNAL_CATCH_REMOVE_PARENS( param ) ) +      \
              1 )
#    endif
...
#    define INTERNAL_CATCH_NTTP_1( signature, ... )                            \
        ...
        template <                                                             \
            template <INTERNAL_CATCH_REMOVE_PARENS( signature )> class...>     \
        struct NttpTemplateTypeList {};                                        \
        template <                                                             \
            template <INTERNAL_CATCH_REMOVE_PARENS( signature )> class... Cs>  \
...
#        define INTERNAL_CATCH_NTTP_REG_METHOD_GEN( TestName, ... ) \
            INTERNAL_CATCH_VA_NARGS_IMPL(                           \
                ...
                INTERNAL_CATCH_NTTP_REGISTER_METHOD0 )( TestName,   \
                                                        __VA_ARGS__ )
...
namespace Catch {
    ...
    auto makeTestInvoker( void ( *testAsFunction )() ) noexcept
        -> ITestInvoker*;
...
#        pragma warning( disable \
                         : 4312 ) // Converting int to T* using reinterpret_cast
...
namespace Catch {
    ...
    struct ITransientExpression {
        ...
        virtual void
        streamReconstructedExpression( std::ostream& os ) const = 0;
...
namespace Catch {
    namespace Matchers {
        ...
        Equals( std::vector<T, AllocComp> const& comparator ) {
            return Vector::EqualsMatcher<T, AllocComp, AllocMatch>(
                comparator );
        }
        ...
        Approx( std::vector<T, AllocComp> const& comparator ) {
            return Vector::ApproxMatcher<T, AllocComp, AllocMatch>(
                comparator );
        }
...
#    define CATCH_INTERNAL_ERROR( ... )             \
        Catch::throw_logic_error(                   \
            CATCH_MAKE_MSG( CATCH_INTERNAL_LINEINFO \
    ...
    namespace Generators {
        ...
        template <typename T> GeneratorWrapper<T> value( T&& value ) {
            return GeneratorWrapper<T>(
                pf::make_unique<SingleValueGenerator<T>>(
                    std::forward<T>( value ) ) );
        }
        template <typename T>
    ...
    namespace Generators {
        ...
        chunk( size_t size, GeneratorWrapper<T>&& generator ) {
            return GeneratorWrapper<std::vector<T>>(
                pf::make_unique<ChunkGenerator<T>>( size,
                                                    std::move( generator ) ) );
        }
...
namespace Catch {
    ...
    private:
        friend bool operator==( SimplePcg32 const& lhs,
...
#        define OC_TEST_CASE2( name, desc, uniqueSuffix )             \
            +(NSString*)OC_MAKE_UNIQUE_NAME( Catch_Name_test_,        \
                                             uniqueSuffix ) {         \
                return @name;                                         \
            }                                                         \
...
namespace Catch {
    namespace Benchmark {
        namespace Detail {
            ...
            template <typename Clock, typename Fun>
            using run_for_at_least_argument_t = typename std::conditional<
                is_callable<Fun( Chronometer )>::value,
                Chronometer,
                int>::type;
    ...
    namespace Benchmark {
        namespace Detail {
            template <typename Clock> std::vector<double> resolution( int k ) {
                ...
                times.reserve( k + 1 );
                std::generate_n(
                    std::back_inserter( times ), k + 1, now<Clock>{} );
            ...
            template <typename Clock = default_clock> void run() {
                ...
                CATCH_CATCH_ALL {
                    if ( translateActiveException() !=
                         Detail::benchmarkErrorMsg ) // benchmark errors have
                                                     // been reported, otherwise
                                                     // rethrow.
                        std::rethrow_exception( std::current_exception() );
                }
            }
...
#        define INTERNAL_CATCH_BENCHMARK(                            \
            BenchmarkName, name, benchmarkIndex )                    \
...
#        endif // CATCH_USE_ASYNC
                ...
                double outlier_variance = Detail::outlier_variance(
                    mean_estimate, stddev_estimate, n );
        ...
        bool Approx::equalityComparisonImpl( const double other ) const {
            ...
            return marginComparison( m_value, other, m_margin ) ||
                   marginComparison(
                       m_value,
                       other,
                       m_epsilon * ( m_scale + std::fabs( std::isinf( m_value )
                                                              ? 0
                                                              : m_value ) ) );
        }
...
#    endif
    ...
    void handleExceptionMatchExpr( AssertionHandler& handler,
        ...
        handleExceptionMatchExpr(
            handler, Matchers::Equals( str ), matcherString );
    }
        ...
        namespace Floating {
            ...
            WithinUlpsMatcher::WithinUlpsMatcher( double target,
                ...
                CATCH_ENFORCE( m_type == FloatingPointKind::Double ||
                               ...
                               "Provided ULP is impossibly large for a float "
                               "comparison." );
            }
...
#    endif
            ...
            bool WithinRelMatcher::match( double const& matchee ) const {
                ...
                return marginComparison( matchee,
                                         m_target,
                                         std::isinf( relMargin ) ? 0
                                                                 : relMargin );
            }
    ...
    namespace Matchers {
        ...
        namespace StdString {
            ...
            bool EqualsMatcher::match( std::string const& source ) const {
                return m_comparator.adjustString( source ) ==
                       m_comparator.m_str;
            }
...
#    endif
    ...
    void RunContext::populateReaction( AssertionReaction& reaction ) {
        ...
        reaction.shouldThrow =
            aborting() || ( m_lastAssertionInfo.resultDisposition &
                            ResultDisposition::Normal );
    }
...
#    endif
    ...
    int Session::run() {
        ...
        if ( ( m_configData.waitForKeypress & WaitForKeypress::BeforeExit ) !=
             0 ) {
            Catch::cout()
...
#    ifndef CATCH_CONFIG_NOSTDOUT // If you #define this you must implement
                                  // these functions
        ...
        void SectionTracker::addInitialFilters(
            ...
            if ( !filters.empty() ) {
                ...
                m_filters.emplace_back(
                    "" ); // Root - should never be consulted
                m_filters.emplace_back(
                    "" ); // Test Case - not a section filter
                m_filters.insert(
    ...
    void XmlReporter::testCaseEnded( TestCaseStats const& testCaseStats ) {
        ...
        if ( !testCaseStats.stdOut.empty() )
            m_xml.scopedElement( "StdOut" )
                .writeText( trim( testCaseStats.stdOut ),
                            XmlFormatting::Newline );
        if ( !testCaseStats.stdErr.empty() )
            m_xml.scopedElement( "StdErr" )
                .writeText( trim( testCaseStats.stdErr ),
                            XmlFormatting::Newline );
...
#        if defined( CATCH_CONFIG_WCHAR ) &&                            \
            defined( CATCH_PLATFORM_WINDOWS ) && defined( _UNICODE ) && \
...
#            define CATCH_REQUIRE( ... )                               \
                INTERNAL_CATCH_TEST( "CATCH_REQUIRE",                  \
...
#            define CATCH_CAPTURE( ... )                    \
                INTERNAL_CATCH_CAPTURE(                     \
                    INTERNAL_CATCH_UNIQUE_NAME( capturer ), \
                    "CATCH_CAPTURE",                        \
                    __VA_ARGS__ )
...
#                define CATCH_STATIC_REQUIRE_FALSE( ... ) \
                    CATCH_REQUIRE_FALSE( __VA_ARGS__ )
#            endif
...
#                define REQUIRE_THAT( arg, matcher )                       \
                    INTERNAL_CHECK_THAT( "REQUIRE_THAT",                   \
                                         matcher,                          \
                                         Catch::ResultDisposition::Normal, \
                                         arg )
#            endif // CATCH_CONFIG_DISABLE_MATCHERS
