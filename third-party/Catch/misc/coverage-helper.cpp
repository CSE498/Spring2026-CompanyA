...
int parse_log_file_arg( std::string const& arg ) {
    assert( starts_with( arg, logfile_prefix ) &&
    ...
    if ( std::regex_search( fname, match, regex ) ) {
        ...
        throw std::domain_error(
            "Couldn't find desired expression in string: " + fname );
    }
}
...
int exec_cmd( std::string const& cmd, int log_num, std::string const& path ) {
    ...
    auto real_cmd = "OpenCppCoverage --export_type binary:cov-report" +
                    std::to_string( log_num ) + ".bin --quiet " + "--sources " +
                    escape_arg( path ) + " --cover_children -- " + cmd;
...
int main( int argc, char** argv ) {
    ...
    try {
        return exec_cmd(
            cmdline, num, windowsify_path( catch_path( args[0] ) ) );
