...
namespace {
    ...
    public:
        ...
        static void advance( int ticks = 1 ) { tick() += ticks; }
...
TEST_CASE( "warmup", "[benchmark]" ) {
    ...
    REQUIRE( ( iterations * rate ) >
             Catch::Benchmark::Detail::warmup_time.count() );
...
TEST_CASE( "resolution", "[benchmark]" ) {
    ...
    size_t count = 10;
    auto res = Catch::Benchmark::Detail::resolution<counting_clock>(
...
TEST_CASE( "uniform samples", "[benchmark]" ) {
    ...
    auto e = Catch::Benchmark::Detail::bootstrap(
        0.95, samples.begin(), samples.end(), samples, []( it a, it b ) {
...
TEST_CASE( "weighted_average_quantile", "[benchmark]" ) {
    ...
    auto q1 = Catch::Benchmark::Detail::weighted_average_quantile(
        1, 4, x.begin(), x.end() );
    auto med = Catch::Benchmark::Detail::weighted_average_quantile(
        1, 2, x.begin(), x.end() );
    auto q3 = Catch::Benchmark::Detail::weighted_average_quantile(
        3, 4, x.begin(), x.end() );
...
TEST_CASE( "classify_outliers", "[benchmark]" ) {
    auto require_outliers = []( Catch::Benchmark::OutlierClassification o,
                                int los,
                                int lom,
                                int him,
                                int his ) {
    ...
    SECTION( "none" ) {
        ...
        auto o =
            Catch::Benchmark::Detail::classify_outliers( x.begin(), x.end() );
...
TEST_CASE( "analyse", "[benchmark]" ) {
    ...
    using Duration =
        Catch::Benchmark::FloatDuration<Catch::Benchmark::default_clock>;
    ...
    auto analysis = Catch::Benchmark::Detail::analyse(
        config, env, samples.begin(), samples.end() );
...
TEST_CASE( "run_for_at_least, chronometer", "[benchmark]" ) {
    ...
    int old_runs = 1;
    auto Timing = Catch::Benchmark::Detail::run_for_at_least<manual_clock>(
        time, 1, [&old_runs]( Catch::Benchmark::Chronometer meter ) -> int {
            CHECK( meter.runs() >= old_runs );
            manual_clock::advance( 100 );
            meter.measure( [] { manual_clock::advance( 1 ); } );
            old_runs = meter.runs();
...
TEST_CASE( "measure", "[benchmark]" ) {
    auto r = Catch::Benchmark::Detail::measure<manual_clock>(
        []( int x ) -> int {
            CHECK( x == 17 );
        ...
        17 );
    auto s = Catch::Benchmark::Detail::measure<manual_clock>(
        []( int x ) -> int {
            CHECK( x == 23 );
...
TEST_CASE( "run benchmark", "[benchmark]" ) {
    ...
    Catch::Benchmark::Benchmark bench{
        "Test Benchmark", []( Catch::Benchmark::Chronometer meter ) {
