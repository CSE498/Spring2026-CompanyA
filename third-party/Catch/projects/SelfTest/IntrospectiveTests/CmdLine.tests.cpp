...
inline Catch::TestCase fakeTestCase( const char* name, const char* desc = "" ) {
    return Catch::makeTestCase(
        nullptr, "", { name, desc }, CATCH_INTERNAL_LINEINFO );
}
...
TEST_CASE( "Parse test names and tags", "[command-line][test-spec]" ) {
    ...
    SECTION( "One tag exclusion, using exclude:, and one wldcarded name "
             "inclusion" ) {
...
TEST_CASE( "#1905 -- test spec parser properly clears internal state between "
           "compound tests",
           "[command-line][test-spec]" ) {
    using Catch::parseTestSpec;
    using Catch::TestSpec;
    // We ask for one of 2 different tests and the latter one of them has a , in
...
TEST_CASE( "#1912 -- test spec parser handles escaping",
           "[command-line][test-spec]" ) {
    using Catch::parseTestSpec;
    using Catch::TestSpec;
    ...
    SECTION( "Various parentheses" ) {
        ...
        REQUIRE( spec.matches( fakeTestCase( R"(spec [a] char)" ) ) );
        REQUIRE_FALSE( spec.matches(
            fakeTestCase( "differs but has similar tag", "[a]" ) ) );
    }
...
TEST_CASE( "Process can be configured on command line",
    ...
    SECTION( "test lists" ) {
        SECTION( "Specify one test case using" ) {
            ...
            REQUIRE( cfg.testSpec().matches( fakeTestCase( "notIncluded" ) ) ==
                     false );
    ...
    SECTION( "abort" ) {
...
#ifndef CATCH_CONFIG_DISABLE_MATCHERS
            REQUIRE_THAT( result.errorMessage(),
                          Contains( "convert" ) && Contains( "oops" ) );
#endif
        }
        ...
        SECTION( "wait-for-keypress" ) {
            SECTION( "Accepted options" ) {
                using tuple_type =
                    std::tuple<char const*, Catch::WaitForKeypress::When>;
                auto input = GENERATE( table<char const*,
                    ...
                    tuple_type{ "both",
                                Catch::WaitForKeypress::BeforeStartAndExit },
                } ) );
                CHECK( cli.parse(
                    { "test", "--wait-for-keypress", std::get<0>( input ) } ) );
            ...
            SECTION( "invalid options are reported" ) {
                auto result =
                    cli.parse( { "test", "--wait-for-keypress", "sometimes" } );
                CHECK( !result );

#ifndef CATCH_CONFIG_DISABLE_MATCHERS
                REQUIRE_THAT( result.errorMessage(),
                              Contains( "never" ) && Contains( "both" ) );
#endif
            }
        }
    }
    ...
    SECTION( "use-colour" ) {
        ...
        SECTION( "error" ) {
            ...
            CHECK( !result );
#ifndef CATCH_CONFIG_DISABLE_MATCHERS
            CHECK_THAT( result.errorMessage(),
    ...
    SECTION( "Benchmark options" ) {
        ...
        SECTION( "confidence-interval" ) {
            CHECK( cli.parse(
                { "test", "--benchmark-confidence-interval=0.99" } ) );

            REQUIRE( config.benchmarkConfidenceInterval ==
                     Catch::Detail::Approx( 0.99 ) );
        }
