...
static const char* const leading_whitespace =
    " \r \t\n There is no extra whitespace here";
static const char* const trailing_whitespace =
    "There is no extra whitespace here \t \n \r ";
...
TEST_CASE( "Trim strings", "[string-manip]" ) {
    ...
    using Catch::trim;
    static_assert(
        std::is_same<std::string, decltype( trim( std::string{} ) )>::value,
        "Trimming std::string should return std::string" );
    static_assert(
        std::is_same<StringRef, decltype( trim( StringRef{} ) )>::value,
        "Trimming StringRef should return StringRef" );
    ...
    REQUIRE( trim( StringRef( leading_whitespace ) ) ==
             StringRef( no_whitespace ) );
    REQUIRE( trim( StringRef( trailing_whitespace ) ) ==
             StringRef( no_whitespace ) );
    REQUIRE( trim( StringRef( whitespace_at_both_ends ) ) ==
             StringRef( no_whitespace ) );
}
...
TEST_CASE( "splitString", "[string-manip]" ) {
    ...
    CHECK_THAT( splitStringRef( "abc", ',' ),
                Equals( std::vector<StringRef>{ "abc" } ) );
    CHECK_THAT( splitStringRef( "abc,def", ',' ),
                Equals( std::vector<StringRef>{ "abc", "def" } ) );
}
