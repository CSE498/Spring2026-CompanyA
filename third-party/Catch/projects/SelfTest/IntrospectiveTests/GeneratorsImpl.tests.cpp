...
TEST_CASE( "Generators internals", "[generators][internals]" ) {
    ...
    SECTION( "Filter generator" ) {
        // Normal usage
        auto gen = filter( []( int i ) { return i != 2; },
                           values( { 2, 1, 2, 3, 2, 2 } ) );
        REQUIRE( gen.get() == 1 );
        REQUIRE( gen.next() );
        REQUIRE( gen.get() == 3 );
        ...
        REQUIRE_THROWS_AS( filter( []( int ) { return false; }, value( 1 ) ),
    ...
    SECTION( "Map with explicit return type" ) {
        auto gen = map<double>( []( int i ) { return 2.0 * i; },
                                values( { 1, 2, 3 } ) );
        REQUIRE( gen.get() == 2.0 );
        REQUIRE( gen.next() );
        REQUIRE( gen.get() == 4.0 );
    ...
    SECTION( "Map with deduced return type" ) {
        auto gen =
            map( []( int i ) { return 2.0 * i; }, values( { 1, 2, 3 } ) );
        REQUIRE( gen.get() == 2.0 );
        REQUIRE( gen.next() );
        REQUIRE( gen.get() == 4.0 );
        REQUIRE( gen.next() );
...
namespace {
    ...
    public:
        TestGen( non_copyable const& nc ): current_number( nc.value ) {}
    ...
    int const& TestGen::get() const { return current_number; }
...
TEST_CASE( "GENERATE capture macros", "[generators][internals][approvals]" ) {
    ...
    auto value2 = GENERATE_REF( Catch::Generators::GeneratorWrapper<int>(
        std::unique_ptr<Catch::Generators::IGenerator<int>>(
            new TestGen( nc ) ) ) );
    REQUIRE( value == value2 );
}

TEST_CASE( "#1809 - GENERATE_COPY and SingleValueGenerator does not compile",
           "[generators][compilation][approvals]" ) {
...
TEST_CASE(
    ...
    "[generators][internals][approvals]" ) {
    SECTION( "Integer" ) {
        ...
        // 0.5% seems like a sane bound for random identical elements within
        // 1000 runs
        REQUIRE( same < 5 );
    }
    SECTION( "Float" ) {
