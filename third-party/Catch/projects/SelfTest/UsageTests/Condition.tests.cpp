...
#ifndef CONDITION_TEST_HELPERS_INCLUDED // Don't compile this more than once per
                                        // TU
#    define CONDITION_TEST_HELPERS_INCLUDED
        ...
        // The "failing" tests all use the CHECK macro, which continues if the
        // specific test fails. This allows us to see all results, even if an
        // earlier check fails
        ...
        TEST_CASE( "Equality checks that should fail",
                   "[.][failing][!mayfail]" ) {
            TestData data;
        ...
        // Needed to test junit reporter's handling of mayfail test cases and
        // sections
        ...
        TEST_CASE( "Inequality checks that should fail",
                   "[.][failing][!shouldfail]" ) {
            TestData data;
        ...
        TEST_CASE( "Ordering comparison checks that should fail",
                   "[.][failing]" ) {
            TestData data;
...
#    pragma clang diagnostic pop
        ...
        TEST_CASE( "Comparisons with int literals don't warn when mixing "
                   "signed/ unsigned" ) {
            int i = 1;
            unsigned int ui = 2;
            long l = 3;
...
#    pragma warning( disable : 4389 ) // '==' : signed/unsigned mismatch
        ...
        TEST_CASE( "Comparisons between unsigned ints and negative signed ints "
                   "match c++ standard behaviour" ) {
            CHECK( ( -1 > 2u ) );
            CHECK( -1 > 2u );
...
#    pragma GCC diagnostic pop
        ...
        // Not (!) tests
        // The problem with the ! operator is that it has right-to-left
        // associativity. This means we can't isolate it when we decompose. The
        // simple REQUIRE( !false ) form, therefore, cannot have the operand
        // value extracted. The test will work correctly, and the situation is
        // detected and a warning issued. An alternative form of the macros
        // (CHECK_FALSE and REQUIRE_FALSE) can be used instead to capture the
        // operand value.
