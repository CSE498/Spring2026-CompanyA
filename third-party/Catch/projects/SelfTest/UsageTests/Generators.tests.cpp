...
TEST_CASE( "Generators -- simple", "[generators]" ) {
    ...
    SECTION( "two" ) {
        // You can also explicitly set type for generators via
        // Catch::Generators::as
...
TEST_CASE( "tables", "[generators]" ) {
    ...
    auto data =
        GENERATE( table<char const*, int>( { tuple_type{ "first", 5 },
                                             tuple_type{ "second", 6 },
                                             tuple_type{ "third", 5 },
                                             tuple_type{ "etc...", 6 } } ) );

    REQUIRE( strlen( std::get<0>( data ) ) ==
             static_cast<size_t>( std::get<1>( data ) ) );
}
...
// https://docs.cucumber.io/gherkin/reference/#scenario-outline (thanks to
// https://github.com/catchorg/Catch2/issues/850#issuecomment-399504851)

// Note that GIVEN, WHEN, and THEN now forward onto DYNAMIC_SECTION instead of
// SECTION. DYNAMIC_SECTION takes its name as a stringstream-style expression,
// so can be formatted using variables in scope - such as the generated
// variables here. This reads quite nicely in the test name output (the full
...
TEST_CASE( "Generators -- adapters", "[generators][generic]" ) {
    ...
    SECTION( "Filtering by predicate" ) {
        SECTION( "Basic usage" ) {
            ...
            auto i = GENERATE( filter( []( int val ) { return val % 2 == 0; },
                                       values( { 1, 2, 3, 4, 5, 6 } ) ) );
            REQUIRE( i % 2 == 0 );
        }
        SECTION( "Throws if there are no matching values" ) {
            ...
            REQUIRE_THROWS_AS(
                filter( []( int ) { return false; }, value( 1 ) ),
                Catch::GeneratorException );
        }
    }
    SECTION( "Shortening a range" ) {
        // This takes the first 3 elements from the values, giving back [1, 2,
        // 3]
    ...
    SECTION( "Transforming elements" ) {
        SECTION( "Same type" ) {
            ...
            auto i = GENERATE( map( []( int val ) { return val * 2; },
                                    values( { 1, 2, 3 } ) ) );
            REQUIRE( i % 2 == 0 );
        }
        SECTION( "Different type" ) {
            // This takes a generator that returns ints and maps them into
            // strings
            auto i = GENERATE( map<std::string>(
    ...
    SECTION( "Chunking a generator into sized pieces" ) {
        SECTION( "Number of elements in source is divisible by chunk size" ) {
            auto chunk2 =
                GENERATE( chunk( 2, values( { 1, 1, 2, 2, 3, 3 } ) ) );
        ...
        SECTION( "Throws on too small generators" ) {
            using namespace Catch::Generators;
            REQUIRE_THROWS_AS( chunk( 2, value( 1 ) ),
                               Catch::GeneratorException );
        }
    }
}
...
TEST_CASE( "Nested generators and captured variables", "[generators]" ) {
    ...
    auto extent = GENERATE( table<int, int>(
        { record{ 3, 7 }, record{ -5, -3 }, record{ 90, 100 } } ) );
...
namespace {
    size_t call_count = 0;
    size_t test_count = 0;
    std::vector<int> make_data() { return { 1, 3, 5, 7, 9, 11 }; }
...
TEST_CASE( "#1913 - GENERATE inside a for loop should not keep recreating the "
           "generator",
