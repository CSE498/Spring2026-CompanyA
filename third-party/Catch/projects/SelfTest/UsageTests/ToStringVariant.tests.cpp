...
TEST_CASE( "variant -- valueless-by-exception",
           "[toString][variant][approvals]" ) {
...
TEST_CASE( "variant<variant<float, int>, string>",
           "[toString][variant][approvals]" ) {
    ...
    SECTION( "valueless nested variant" ) {
        ...
        REQUIRE_THROWS_AS( std::get<0>( value ).emplace<MyType1>( MyType1{} ),
                           int );
...
TEST_CASE( "variant<nullptr,int,const char *>",
           "[toString][variant][approvals]" ) {
