...
#    define MISC_TEST_HELPERS_INCLUDED
        ...
        struct AutoTestReg {
            AutoTestReg() {
                REGISTER_TEST_CASE( manuallyRegisteredTestFunction,
                                    "ManuallyRegistered" );
            }
        };
        ...
        TEST_CASE( "more nested SECTION tests", "[sections][failing][.]" ) {
            ...
            SECTION( "doesn't equal" ) {
                SECTION( "equal" ) { REQUIRE( a == b ); }

                SECTION( "not equal" ) { REQUIRE( a != b ); }
                SECTION( "less than" ) { REQUIRE( a < b ); }
            }
        }
        ...
        TEST_CASE( "looped SECTION tests", "[.][failing][sections]" ) {
            ...
            for ( int b = 0; b < 10; ++b ) {
                DYNAMIC_SECTION( "b is currently: " << b ) { CHECK( b > a ); }
            }
        }
        ...
        TEST_CASE( "xmlentitycheck" ) {
            SECTION( "embedded xml: <test>it should be possible to embed xml "
                     "characters, such as <, \" or &, or even whole "
                     "<xml>documents</xml> within an attribute</test>" ) {
                SUCCEED(); // We need this here to stop it failing due to no
                           // tests
            }
        ...
        //    char line[200];
        //    FILE* output = popen("./CatchSelfTest
        //    ./failing/matchers/StartsWith", "r"); while ( fgets(line, 199,
        //    output) )
        //        std::cout << line;
        //}
        ...
        TEMPLATE_TEST_CASE( "TemplateTest: vectors can be sized and resized",
                            "[vector][template]",
                            int,
                            float,
                            std::string,
        ...
        TEMPLATE_TEST_CASE_SIG(
            "TemplateTestSig: vectors can be sized and resized",
            "[vector][template][nttp]",
            ( ( typename TestType, int V ), TestType, V ),
            ( int, 5 ),
            ( float, 4 ),
            ( std::string, 15 ),
        ...
        TEMPLATE_LIST_TEST_CASE(
            "Template test case with test types specified inside std::tuple",
            "[template][list]",
            MyTypes ) {
        ...
        using MyNonDefaultConstructibleTypes =
            std::tuple<NonDefaultConstructibleType, float>;
        TEMPLATE_LIST_TEST_CASE( "Template test case with test types specified "
                                 "inside non-default-constructible std::tuple",
                                 "[template][list]",
                                 MyNonDefaultConstructibleTypes ) {
            REQUIRE( sizeof( TestType ) > 0 );
        }
        ...
        struct NonCopyableAndNonMovableType {
            ...
            NonCopyableAndNonMovableType(
                NonCopyableAndNonMovableType const& ) = delete;
            NonCopyableAndNonMovableType( NonCopyableAndNonMovableType&& ) =
                delete;
            auto operator=( NonCopyableAndNonMovableType const& )
                -> NonCopyableAndNonMovableType& = delete;
            auto operator=( NonCopyableAndNonMovableType&& )
                -> NonCopyableAndNonMovableType& = delete;
        };

        using NonCopyableAndNonMovableTypes =
            std::tuple<NonCopyableAndNonMovableType, float>;
        TEMPLATE_LIST_TEST_CASE(
            "Template test case with test types specified inside non-copyable "
            "and non-movable std::tuple",
            "[template][list]",
            NonCopyableAndNonMovableTypes ) {
            REQUIRE( sizeof( TestType ) > 0 );
        }

        // https://github.com/philsquared/Catch/issues/166
        TEST_CASE( "A couple of nested sections followed by a failure",
                   "[failing][.]" ) {
            SECTION( "Outer" )
            SECTION( "Inner" )
        ...
        TEST_CASE( "not allowed", "[!throws]" ) {
            // This test case should not be included if you run with -e on the
            // command line
            SUCCEED();
        }
        ...
        TEST_CASE( "Tabs and newlines show in output",
                   "[.][whitespace][failing]" ) {
        ...
        TEST_CASE( "toString on const wchar_t const pointer returns the string "
                   "contents",
                   "[toString]" ) {
        ...
        TEST_CASE( "toString on wchar_t returns the string contents",
                   "[toString]" ) {
        ...
        TEST_CASE( "This test 'should' fail but doesn't",
                   "[.][failing][!shouldfail]" ) {
            SUCCEED( "oops!" );
        }
        ...
        TEMPLATE_TEST_CASE_SIG( "#1954 - 7 arg template test case sig compiles",
