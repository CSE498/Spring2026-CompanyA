...
TEST_CASE( "INFO and WARN do not abort tests", "[messages][.]" ) {
    ...
    WARN( "this is a " << "warning" ); // This should always output the message
                                       // but then continue
}

TEST_CASE( "#1455 - INFO and WARN can start with a linebreak",
           "[messages][.]" ) {
...
TEST_CASE( "INFO gets logged on failure, even if captured before successful "
           "assertions",
           "[failing][messages][.]" ) {
...
TEST_CASE( "FAIL aborts the test", "[failing][messages][.]" ) {
    FAIL(
        "This is a " << "failure" ); // This should output the message and abort
...
TEST_CASE( "Standard error is reported and redirected",
           "[messages][.][approvals]" ) {
    SECTION( "std::cerr" ) { std::cerr << "Write to std::cerr" << std::endl; }
    SECTION( "std::clog" ) { std::clog << "Write to std::clog" << std::endl; }
...
TEST_CASE( "Pointers can be converted to strings",
           "[messages][.][approvals]" ) {
    int p;
...
template <typename T> static void unscoped_info( T msg ) {
    UNSCOPED_INFO( msg );
}
...
TEST_CASE( "just failure after unscoped info",
           "[failing][.][unscoped][info]" ) {
    FAIL( "previous unscoped info SHOULD not be seen" );
}

TEST_CASE( "print unscoped info if passing unscoped info is printed",
           "[unscoped][info]" ) {
    unscoped_info(
...
TEST_CASE( "not prints unscoped info from previous failures",
           "[failing][.][unscoped][info]" ) {
    unscoped_info( "this MAY be seen only for the FIRST assertion IF info is "
                   "printed for passing assertions" );
    REQUIRE( true );
    unscoped_info( "this MAY be seen only for the SECOND assertion IF info is "
                   "printed for passing assertions" );
    REQUIRE( true );
    unscoped_info( "this SHOULD be seen" );
    REQUIRE( false );
...
TEST_CASE( "prints unscoped info only for the first assertion",
           "[failing][.][unscoped][info]" ) {
    unscoped_info( "this SHOULD be seen only ONCE" );
    CHECK( false );
    CHECK( true );
    unscoped_info( "this MAY also be seen only ONCE IF info is printed for "
                   "passing assertions" );
    CHECK( true );
    CHECK( true );
}
...
TEST_CASE( "CAPTURE can deal with complex expressions",
           "[messages][capture]" ) {
    int a = 1;
    int b = 2;
    int c = 3;
...
#    pragma clang diagnostic ignored \
        "-Wunused-value" // In (1, 2), the "1" is unused ...
#endif
#ifdef __GNUC__
#    pragma GCC diagnostic push
#    pragma GCC diagnostic ignored \
        "-Wunused-value" // All the comma operators are side-effect free
#endif
#ifdef _MSC_VER
#    pragma warning( push )
...
template <typename T1, typename T2> struct helper_1436 {
    helper_1436( T1 t1_, T2 t2_ ): t1{ t1_ }, t2{ t2_ } {}
    T1 t1;
    T2 t2;
};
...
std::ostream& operator<<( std::ostream& out,
                          helper_1436<T1, T2> const& helper ) {
...
TEST_CASE( "CAPTURE parses string and character constants",
           "[messages][capture]" ) {
    CAPTURE( ( "comma, in string", "escaped, \", " ),
