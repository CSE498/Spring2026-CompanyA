...
#ifndef COMPILATION_TEST_HELPERS_INCLUDED // Don't compile this more than once
                                          // per TU
#    define COMPILATION_TEST_HELPERS_INCLUDED
        ...
        template <typename T> bool operator==( const T& val, foo f ) {
            return val == f.i;
        }
        ...
        template <typename T> bool templated_tests( T t ) {
            int a = 3;
            REQUIRE( a == t );
            CHECK( a == t );
...
#    endif
#    ifdef __GNUC__
// Note that because -~GCC~-, this warning cannot be silenced temporarily, by
// pushing diagnostic stack... Luckily it is firing in test files and thus can
// be silenced for the whole file, without losing much.
...
#    endif
        ...
        template <typename, typename> struct Fixture_1245 {};
...
#endif
        TEST_CASE( "#925: comparing function pointer to function address "
                   "failed to compile",
                   "[!nonportable]" ) {
            TestClass test;
            REQUIRE( utility::synchronizing_callback !=
...
namespace {
    struct HasBitOperators {
        ...
        friend HasBitOperators operator|( HasBitOperators lhs,
                                          HasBitOperators rhs ) {
...
TEST_CASE( "Assertion macros support bit operators and bool conversions",
           "[compilation][bitops]" ) {
