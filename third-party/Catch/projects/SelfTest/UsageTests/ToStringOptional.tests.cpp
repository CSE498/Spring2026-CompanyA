...
TEST_CASE( "std::optional<int> -> toString",
...
TEST_CASE( "std::optional<std::string> -> toString",
           "[toString][optional][approvals]" ) {
...
TEST_CASE( "std::vector<std::optional<int> > -> toString",
           "[toString][optional][approvals]" ) {
    using type = std::vector<std::optional<int>>;
    REQUIRE( "{ 0, { }, 2 }" ==
             ::Catch::Detail::stringify( type{ 0, {}, 2 } ) );
}
