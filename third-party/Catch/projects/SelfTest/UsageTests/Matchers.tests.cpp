...
#    ifndef MATCHERS_TEST_HELPERS_INCLUDED // Don't compile this more than once
                                           // per TU
#        define MATCHERS_TEST_HELPERS_INCLUDED
        ...
        inline const char* testStringForMatching2() {
            return "some completely different text that contains one common "
                   "word";
        }
...
#            pragma warning( disable \
                             : 4702 ) // Unreachable code -- MSVC 19 (VS 2015)
...
#    endif
        ...
        // <regex> does not work in libstdc++ 4.8, so we have to enable these
        // tests only when they are expected to pass and cannot have them in
        // baselines
...
#    endif
        ...
        TEST_CASE( "Vector matchers", "[matchers][vector]" ) {
            ...
            SECTION( "Equals" ) {
                ...
                CHECK_THAT(
                    v5,
                    ( Equals<int, std::allocator<int>, CustomAllocator<int>>(
                        v2 ) ) );
            ...
            SECTION( "UnorderedEquals" ) {
                ...
                CHECK_THAT(
                    ...
                    ( UnorderedEquals<int,
                                      std::allocator<int>,
                                      CustomAllocator<int>>( permuted ) ) );

                auto v5_permuted = v5;
                std::next_permutation( begin( v5_permuted ),
                                       end( v5_permuted ) );
                CHECK_THAT( v5_permuted, UnorderedEquals( v5 ) );
            }
        }

        TEST_CASE( "Vector matchers that fail",
                   "[matchers][vector][.][failing]" ) {
            std::vector<int> v;
            v.push_back( 1 );
            v.push_back( 2 );
            v.push_back( 3 );
        ...
        TEST_CASE( "Exception matchers that succeed",
            ...
            CHECK_THROWS_MATCHES( throwsSpecialException( 1 ),
                                  SpecialException,
                                  ExceptionMatcher{ 1 } );
        ...
        TEST_CASE( "Floating point matchers: float",
            ...
            SECTION( "Composed" ) {
                REQUIRE_THAT( 1.f,
                              WithinAbs( 1.f, 0.5 ) || WithinULP( 1.f, 1 ) );
                REQUIRE_THAT( 1.f,
                              WithinAbs( 2.f, 0.5 ) || WithinULP( 1.f, 0 ) );
                REQUIRE_THAT( 0.0001f,
                              WithinAbs( 0.f, 0.001f ) ||
                                  WithinRel( 0.f, 0.1f ) );
            }
            SECTION( "Constructor validation" ) {
                ...
                REQUIRE_THROWS_AS(
                    WithinULP( 1.f, static_cast<uint64_t>( -1 ) ),
                    std::domain_error );
        ...
        TEST_CASE( "Floating point matchers: double",
                   "[matchers][floating-point]" ) {
            SECTION( "Relative" ) {
            ...
            SECTION( "Composed" ) {
                ...
                REQUIRE_THAT( 0.0001,
                              WithinAbs( 0., 0.001 ) || WithinRel( 0., 0.1 ) );
            }
            SECTION( "Constructor validation" ) {
        ...
        TEST_CASE( "Floating point matchers that are problematic in approvals",
            ...
            REQUIRE_THAT( NAN,
                          !( WithinAbs( NAN, 100 ) || WithinULP( NAN, 123 ) ) );
        ...
        TEST_CASE( "Arbitrary predicate matcher", "[matchers][generic]" ) {
            SECTION( "Function pointer" ) {
                ...
                REQUIRE_THAT( 1,
                              !Predicate<int>( alwaysFalse, "always false" ) );
            }
        ...
        TEST_CASE( "Vector Approx matcher", "[matchers][approx][vector]" ) {
            ...
            SECTION( "Vectors with elements" ) {
                ...
                SECTION( "Same length, different elements" ) {
                    ...
                    REQUIRE_THAT( v1,
                                  Approx( v2 ).epsilon( 0.1 ).scale( 500 ) );
                }
            }
        }
