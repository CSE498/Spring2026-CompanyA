...
std::string ws( int const level ) { return std::string( 2 * level, ' ' ); }
...
void print( std::ostream& os,
    ...
    print( os, level + 1, "- lineInfo", info.lineInfo );
    os << ws( level + 1 ) << "- properties (flags): 0x" << std::hex
       << info.properties << std::dec << "\n";
}
...
void print( std::ostream& os,
    ...
    print( os, level + 1, "- lineInfo", info.lineInfo );
    os << ws( level + 1 ) << "- capturedExpression: '"
       << info.capturedExpression << "'\n"
       << ws( level + 1 ) << "- resultDisposition (flags): 0x" << std::hex
       << info.resultDisposition << std::dec << "\n";
}
...
void print( std::ostream& os,
    ...
    os << ws( level ) << title << ":\n"
       << ws( level + 1 ) << "- reconstructExpression(): '"
...
void print( std::ostream& os,
    ...
    os << ws( level ) << title << ":\n"
       ...
       << "'\n"
       << ws( level + 1 ) << "- getExpressionInMacro(): '"
    ...
    os << ws( level + 1 ) << "- getTestMacroName(): '"
       << info.getTestMacroName() << "'\n";

    //    print( os, level+1 , "- *** m_info (AssertionInfo)", info.m_info );
    //    print( os, level+1 , "- *** m_resultData (AssertionResultData)",
    //    info.m_resultData );
}
...
struct MyListener : Catch::TestEventListenerBase {
    ...
    void
    assertionStarting( Catch::AssertionInfo const& assertionInfo ) override {
        std::cout << "\nEvent: assertionStarting:\n";
        print( std::cout, 1, "- assertionInfo", assertionInfo );
    }

    bool
    assertionEnded( Catch::AssertionStats const& assertionStats ) override {
...
// Compile & run:
// - g++ -std=c++11 -Wall -I$(CATCH_SINGLE_INCLUDE) -o 210-Evt-EventListeners
// 210-Evt-EventListeners.cpp 000-CatchMain.o && 210-Evt-EventListeners
// --success
// - cl -EHsc -I%CATCH_SINGLE_INCLUDE% 210-Evt-EventListeners.cpp
// 000-CatchMain.obj && 210-Evt-EventListeners --success
