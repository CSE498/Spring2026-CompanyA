...
struct MyException : public std::runtime_error {
    MyException( char const* text ): std::runtime_error( text ) {}
...
struct MyExceptionMatcher : Catch::MatcherBase<std::runtime_error> {
    ...
    MyExceptionMatcher( char const* text ): m_text( text ) {}
    ...
    std::string describe() const override { return "it's me"; }
};
...
TEST_CASE( "TeamCity failing check-throws-matches", "[teamcity]" ) {
    ...
    CHECK_THROWS_MATCHES( throw MyException( "hello" ),
                          MyException,
                          MyExceptionMatcher( "world" ) );
}

// [!throws] - lets Catch know that this test is likely to throw an exception
// even if successful. This causes the test to be excluded when running with -e
// or --nothrow.
...
// [!mayfail] - doesn't fail the test if any given assertion fails (but still
// reports it). This can be useful to flag a work-in-progress, or a known issue
// that you don't want to immediately fix but still want to track in your tests.
...
// This can be useful if you want to be notified of accidental, or third-party,
// fixes.
...
// Compile & run:
// - g++ -std=c++11 -Wall -I$(CATCH_ROOT)
// -DCATCH_EXAMPLE_RPT_1=\"include/reporters/catch_reporter_teamcity.hpp\" -o
// 200-Rpt-CatchMainTeamCity.o -c 200-Rpt-CatchMain.cpp
// - g++ -std=c++11 -Wall -I$(CATCH_ROOT) -o 207-Rpt-TeamCityReporter
// 207-Rpt-TeamCityReporter.cpp 200-Rpt-CatchMainTeamCity.o &&
// 207-Rpt-TeamCityReporter --list-reporters
//
// - cl -EHsc -I%CATCH_ROOT%
// -DCATCH_EXAMPLE_RPT_1=\"include/reporters/catch_reporter_teamcity.hpp\"
// -Fo200-Rpt-CatchMainTeamCity.obj -c 200-Rpt-CatchMain.cpp
// - cl -EHsc -I%CATCH_ROOT% 207-Rpt-TeamCityReporter.cpp
// 200-Rpt-CatchMainTeamCity.o && 207-Rpt-TeamCityReporter --list-reporters
...
// ##teamcity[testStarted name='TeamCity passes unconditionally succeeding
// assertion']
// ##teamcity[testFinished name='TeamCity passes unconditionally succeeding
// assertion' duration='1']
// ##teamcity[testStarted name='TeamCity reports unconditionally failing
// assertion']
// ##teamcity[testFailed name='TeamCity reports unconditionally failing
// assertion' / message='.../examples/207-Rpt-TeamCityReporter.cpp:23|n/
// ...............................................................................|n|n/
// .../examples/207-Rpt-TeamCityReporter.cpp:25|nexplicit failure']
// ##teamcity[testFinished name='TeamCity reports unconditionally failing
